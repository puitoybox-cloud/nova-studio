...
PREVIEW_SHA="3ca4ea338fb389afc44feba6bc5cb6532f10e72b"
PREVIEW_PORT="8765"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
PREVIEW_URL="http://127.0.0.1:${PREVIEW_PORT}/music-studio.html?verification=${PREVIEW_SHA}#music-studio"
PID_FILE="/tmp/nova-studio-pr240-preview.pid"
LOG_FILE="/tmp/nova-studio-pr240-preview.log"

if ! command -v python3 >/dev/null 2>&1; then
  osascript -e 'display alert "PR #240 Previewを起動できません" message "python3が見つかりません。画面を閉じずにティアへ知らせてください。" as critical' 2>/dev/null || true
...
for _ in 1 2 3 4 5; do
  if curl --silent --fail "http://127.0.0.1:${PREVIEW_PORT}/music-studio.html" >/dev/null; then
    ...
    echo "PR #240 Previewを起動しました。"
    echo "対象commit: $PREVIEW_SHA"
    echo "確認URL: $PREVIEW_URL"
    echo "この画面は閉じて構いません。Preview停止時はSTOP_PR240_PREVIEW.commandをダブルクリックしてください。"
    exit 0
  fi
  sleep 1
done
