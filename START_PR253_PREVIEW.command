...
MARKER_URL="http://127.0.0.1:${PREVIEW_PORT}/PR253_EXACT_HEAD.txt?verification=${PRODUCT_SHA}"
PID_FILE="/tmp/nova-studio-pr253-${PRODUCT_SHA}-preview.pid"
LOG_FILE="/tmp/nova-studio-pr253-${PRODUCT_SHA}-preview.log"

if ! command -v python3 >/dev/null 2>&1; then
  osascript -e 'display alert "PR #253 Previewを起動できません" message "python3が見つかりません。画面を閉じずにティアへ知らせてください。" as critical' 2>/dev/null || true
...
if curl --silent --fail "$MARKER_URL" 2>/dev/null | grep --fixed-strings --quiet "Product HEAD: $PRODUCT_SHA"; then
  ...
  echo "指定HEADのPR #253 Previewはすでに起動しています。"
  echo "製品HEAD: $PRODUCT_SHA"
  exit 0
fi

if curl --silent --fail "http://127.0.0.1:${PREVIEW_PORT}/" >/dev/null 2>&1; then
  osascript -e 'display alert "PR #253 Previewを起動できません" message "localhost:8767は別のPreviewに使用されています。別HEADを表示しないため起動を中止しました。既存Previewを停止してから再実行してください。" as critical' 2>/dev/null || true
...
for _ in 1 2 3 4 5; do
  if curl --silent --fail "$MARKER_URL" | grep --fixed-strings --quiet "Product HEAD: $PRODUCT_SHA"; then
    open -a "Google Chrome" "$MARKER_URL" "$PREVIEW_URL" 2>/dev/null || open "$PREVIEW_URL"
    echo "PR #253 exact-head Previewを起動しました。"
    echo "製品HEAD: $PRODUCT_SHA"
    echo "verification marker: $MARKER_URL"
    echo "確認URL: $PREVIEW_URL"
    echo "停止時はSTOP_PR253_PREVIEW.commandをダブルクリックしてください。"
    exit 0
  fi
  sleep 1
done
