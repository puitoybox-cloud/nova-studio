...
if curl --silent --fail "http://127.0.0.1:${PREVIEW_PORT}/" >/dev/null 2>&1; then
  osascript -e 'display alert "PR #253 Previewを起動できません" message "localhost:8768は別のPreviewに使用されています。別HEADを表示しないため起動を中止しました。既存Previewを停止してから再実行してください。" as critical' 2>/dev/null || true
