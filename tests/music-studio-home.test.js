...
test('Music Studio dependencies load sequentially without querying detached scripts',()=>{
  ...
  assert.match(indexSource,/app\.js\?v=1\.5\.56/);
  ...
  assert.match(hostSource,/music-studio\.css\?v=1\.4\.129/);assert.match(standaloneSource,/music-studio\.css\?v=1\.4\.129/);
  assert.match(hostSource,/music-studio\.js\?v=1\.4\.108/);assert.match(standaloneSource,/music-studio\.js\?v=1\.4\.108/);
  ...
  assert.match(hostSource,/MusicStudio\?\.ASSET_VERSION==='1\.4\.108'&&typeof window\.MusicStudio\.externalTrackReviewHtml==='function'/);
  assert.match(hostSource,/window\.MusicStudioRuntime=Object\.freeze\(\{hostLoader:'1\.5\.57',stylesheet:'1\.4\.129'/);
