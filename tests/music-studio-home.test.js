...
test('Music Studio dependencies load sequentially without querying detached scripts',()=>{
  ...
  assert.match(indexSource,/app\.js\?v=1\.5\.54/);
  ...
  assert.match(hostSource,/music-studio\.js\?v=1\.4\.98/);assert.match(standaloneSource,/music-studio\.js\?v=1\.4\.98/);
  ...
  assert.match(hostSource,/MusicStudio\?\.ASSET_VERSION==='1\.4\.98'&&typeof window\.MusicStudio\.externalTrackReviewHtml==='function'/);
