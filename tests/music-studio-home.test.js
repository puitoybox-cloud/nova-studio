...
test('Music Studio dependencies load sequentially without querying detached scripts',()=>{
  ...
  assert.match(indexSource,/app\.js\?v=1\.5\.57/);
  ...
  assert.match(hostSource,/music-studio\.css\?v=1\.4\.130/);assert.match(standaloneSource,/music-studio\.css\?v=1\.4\.130/);
  assert.match(hostSource,/music-studio\.js\?v=1\.4\.109/);assert.match(standaloneSource,/music-studio\.js\?v=1\.4\.109/);
  ...
  assert.match(hostSource,/MusicStudio\?\.ASSET_VERSION==='1\.4\.109'&&typeof window\.MusicStudio\.externalTrackReviewHtml==='function'/);
  assert.match(hostSource,/window\.MusicStudioRuntime=Object\.freeze\(\{hostLoader:'1\.5\.58',stylesheet:'1\.4\.130'/);
