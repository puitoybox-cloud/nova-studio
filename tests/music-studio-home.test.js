...
test('Music Studio dependencies load sequentially without querying detached scripts',()=>{
  ...
  assert.match(indexSource,/app\.js\?v=1\.4\.18/);
  ...
  assert.match(hostSource,/music-studio\.css\?v=1\.4\.23/);assert.match(standaloneSource,/music-studio\.css\?v=1\.4\.23/);
  ...
  assert.match(hostSource,/music-studio\.js\?v=1\.4\.33/);assert.match(standaloneSource,/music-studio\.js\?v=1\.4\.33/);
...
test('Record starts for a connected MIDI input and Stop ends the active recording',async()=>{
  ...
  await app.editorStopTransport();assert.equal(stopped,1);assert.equal(app.state.midiInput.recording,false);assert.match(app.state.midiInput.status,/録音を停止/);
});
