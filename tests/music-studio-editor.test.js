...
test('Gemini no-schema diagnostic is Smoke-only and changes only the final schema subtree',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',config=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-smoke-model'}),calls=[],output={version:1,trackId:'smoke-melody',part:'melody',range:{startMeasure:1,endMeasure:1,startTick:0,endTick:1920},updates:[],adds:[],deletes:[]},fetch=async(...args)=>{calls.push(args);return{ok:true,text:async()=>JSON.stringify({candidates:[{content:{parts:[{text:JSON.stringify(output)}]}}]})}},normal=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch}),diagnostic=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch,geminiDiagnosticMode:'no-schema'}),a=JSON.parse(calls[0][1].body),b=JSON.parse(calls[1][1].body),withoutSchema=JSON.parse(JSON.stringify(a));delete withoutSchema.generationConfig.responseFormat.text.schema;
  assert.deepEqual(JSON.parse(JSON.stringify(normal)),{ok:true,provider:'gemini',model:'gemini-smoke-model'});assert.deepEqual(JSON.parse(JSON.stringify(diagnostic)),JSON.parse(JSON.stringify(normal)));assert.equal(Object.hasOwn(a.generationConfig.responseFormat.text,'schema'),true);assert.equal(Object.hasOwn(b.generationConfig.responseFormat.text,'schema'),false);assert.deepEqual(b,withoutSchema);assert.equal(calls[0][0],calls[1][0]);assert.equal(calls[0][1].method,calls[1][1].method);assert.equal(a.generationConfig.responseFormat.text.mimeType,b.generationConfig.responseFormat.text.mimeType);assert.deepEqual(a.systemInstruction,b.systemInstruction);assert.deepEqual(a.contents,b.contents);assert.equal(JSON.stringify(normal).includes(secret),false);assert.equal(JSON.stringify(diagnostic).includes(secret),false)
});
test('Smoke rejects unknown or non-Gemini diagnostic modes before transport',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',gemini=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-model'}),openai=core.createPartialEditProviderConfig('openai',{apiKey:secret,model:'openai-model'}),calls=[];for(const[config,geminiDiagnosticMode]of [[gemini,'unknown-mode'],[openai,'no-schema']]){const result=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{geminiDiagnosticMode,fetch:async()=>{calls.push(true);throw Error('must not run')}});assert.equal(result.ok,false);assert.equal(result.stage,'config');assert.equal(result.code,'invalid-input');assert.equal(JSON.stringify(result).includes(secret),false)}assert.equal(calls.length,0);
