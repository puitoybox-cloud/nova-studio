...
test('provider transport sends PR11 Gemini payload and returns a PR12 result',async()=>{
  ...
  const body=JSON.parse(calls[0][1].body);assert.equal(calls[0][0],'https://generativelanguage.googleapis.com/v1beta/models/gemini-test:generateContent');assert.equal(calls[0][1].headers['x-goog-api-key'],'gemini-test-secret');assert.equal(JSON.stringify(body.systemInstruction),JSON.stringify(payload.systemInstruction));assert.equal(JSON.stringify(body.contents),JSON.stringify(payload.contents));assert.equal(JSON.stringify(body.generationConfig),JSON.stringify(payload.generationConfig));assert.equal(result.status,'success');assert.deepEqual(JSON.parse(JSON.stringify(core.extractPartialEditProviderOutput(result))),JSON.parse(JSON.stringify(output)));assert.equal(JSON.stringify(result).includes('gemini-test-secret'),false)
});
