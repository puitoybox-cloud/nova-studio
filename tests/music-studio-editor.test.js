...
test('Gemini A B C D E diagnostics are Smoke-only and remove only their target subtrees',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',config=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-smoke-model'}),calls=[],output={version:1,trackId:'smoke-melody',part:'melody',range:{startMeasure:1,endMeasure:1,startTick:0,endTick:1920},updates:[],adds:[],deletes:[]},fetch=async(...args)=>{calls.push(args);return{ok:true,text:async()=>JSON.stringify({candidates:[{content:{parts:[{text:JSON.stringify(output)}]}}]})}},normal=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch}),noSchema=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch,geminiDiagnosticMode:'no-schema'}),noResponseFormat=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch,geminiDiagnosticMode:'no-response-format'}),noGenerationConfig=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch,geminiDiagnosticMode:'no-generation-config'}),noSystemInstruction=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch,geminiDiagnosticMode:'no-system-instruction'}),a=JSON.parse(calls[0][1].body),b=JSON.parse(calls[1][1].body),c=JSON.parse(calls[2][1].body),d=JSON.parse(calls[3][1].body),e=JSON.parse(calls[4][1].body),withoutSchema=JSON.parse(JSON.stringify(a)),withoutResponseFormat=JSON.parse(JSON.stringify(b)),withoutGenerationConfig=JSON.parse(JSON.stringify(c)),withoutSystemInstruction=JSON.parse(JSON.stringify(d));delete withoutSchema.generationConfig.responseFormat.text.schema;delete withoutResponseFormat.generationConfig.responseFormat;delete withoutGenerationConfig.generationConfig;delete withoutSystemInstruction.systemInstruction;
  assert.deepEqual(JSON.parse(JSON.stringify(normal)),{ok:true,provider:'gemini',model:'gemini-smoke-model'});for(const result of [noSchema,noResponseFormat,noGenerationConfig,noSystemInstruction])assert.deepEqual(JSON.parse(JSON.stringify(result)),JSON.parse(JSON.stringify(normal)));assert.equal(Object.hasOwn(a.generationConfig.responseFormat.text,'schema'),true);assert.equal(Object.hasOwn(b.generationConfig.responseFormat.text,'schema'),false);assert.equal(Object.hasOwn(c,'generationConfig'),true);assert.deepEqual(c.generationConfig,{});assert.equal(Object.hasOwn(c.generationConfig,'responseFormat'),false);assert.equal(Object.hasOwn(d,'generationConfig'),false);assert.equal(Object.hasOwn(e,'generationConfig'),false);assert.equal(Object.hasOwn(e,'systemInstruction'),false);assert.equal(Object.hasOwn(e,'contents'),true);assert.deepEqual(b,withoutSchema);assert.deepEqual(c,withoutResponseFormat);assert.deepEqual(d,withoutGenerationConfig);assert.deepEqual(e,withoutSystemInstruction);for(const call of calls){assert.equal(call[0],calls[0][0]);assert.equal(call[1].method,calls[0][1].method)}assert.equal(a.generationConfig.responseFormat.text.mimeType,b.generationConfig.responseFormat.text.mimeType);for(const body of [b,c,d,e])assert.deepEqual(a.contents,body.contents);for(const body of [b,c,d])assert.deepEqual(a.systemInstruction,body.systemInstruction);for(const result of [normal,noSchema,noResponseFormat,noGenerationConfig,noSystemInstruction])assert.equal(JSON.stringify(result).includes(secret),false)
});
test('Smoke rejects unknown or non-Gemini diagnostic modes before transport',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',gemini=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-model'}),openai=core.createPartialEditProviderConfig('openai',{apiKey:secret,model:'openai-model'}),calls=[];for(const[config,geminiDiagnosticMode]of [[gemini,'unknown-mode'],[openai,'no-schema'],[openai,'no-response-format'],[openai,'no-generation-config'],[openai,'no-system-instruction']]){const result=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{geminiDiagnosticMode,fetch:async()=>{calls.push(true);throw Error('must not run')}});assert.equal(result.ok,false);assert.equal(result.stage,'config');assert.equal(result.code,'invalid-input');assert.equal(JSON.stringify(result).includes(secret),false)}assert.equal(calls.length,0);
  const cli=spawnSync(process.execPath,[path.join(__dirname,'..','scripts','music-studio-live-api-smoke.js'),'gemini','unknown-mode'],{encoding:'utf8',env:{...process.env,GEMINI_API_KEY:secret,GEMINI_MODEL:'gemini-model'}});assert.notEqual(cli.status,0);assert.match(cli.stderr,/Usage:/);assert.equal((cli.stdout+cli.stderr).includes(secret),false)
});
