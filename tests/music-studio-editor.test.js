...
test('provider transport sends PR11 Gemini payload and returns a PR12 result',async()=>{
  ...
  const body=JSON.parse(calls[0][1].body),schema=body.generationConfig.responseSchema;assert.equal(calls[0][0],'https://generativelanguage.googleapis.com/v1beta/models/gemini-test:generateContent');assert.equal(calls[0][1].headers['x-goog-api-key'],'gemini-test-secret');assert.equal(JSON.stringify(body.systemInstruction),JSON.stringify(payload.systemInstruction));assert.equal(JSON.stringify(body.contents),JSON.stringify(payload.contents));assert.deepEqual(schema.properties.version.enum,[1]);assert.equal(Object.hasOwn(schema.properties.version,'const'),false);assert.equal(Object.hasOwn(schema.properties.range.properties.endTick,'exclusiveMinimum'),false);assert.equal(payload.generationConfig.responseSchema.properties.version.const,1);assert.equal(payload.generationConfig.responseSchema.properties.range.properties.endTick.exclusiveMinimum,0);assert.equal(result.status,'success');assert.deepEqual(JSON.parse(JSON.stringify(core.extractPartialEditProviderOutput(result))),JSON.parse(JSON.stringify(output)));assert.equal(JSON.stringify(result).includes('gemini-test-secret'),false)
});
...
test('provider transport preserves only sanitized HTTP diagnostics',async()=>{
  const core=load(),{request}=adapterFixture(core),input=core.createPartialEditAIInput(request),instruction=core.createPartialEditInstruction({intent:'darker'}),context=core.createPartialEditPromptContext(request,input,instruction),payload=core.createPartialEditProviderPayload('gemini',context),secret='never-leak-http-key',categories=new Map([[400,'request-schema-failure'],[401,'authentication-failure'],[403,'permission-failure'],[404,'model-unavailable'],[429,'rate-limit'],[500,'provider-server-error']]);
  for(const[status,category]of categories){let bodyRead=false;const result=await core.executePartialEditProviderRequest('gemini',payload,{apiKey:secret,model:'gemini-test',fetch:async()=>({ok:false,status,text:async()=>{bodyRead=true;return`raw ${secret}`}})});assert.equal(bodyRead,false);assert.deepEqual(JSON.parse(JSON.stringify(result.error)),{code:'provider-error',message:'The provider did not return a usable result.',httpStatus:status,category});assert.equal(core.validatePartialEditProviderExecutionResult(result).ok,true);assert.equal(JSON.stringify(result).includes(secret),false)}
});
...
test('live API smoke exposes sanitized transport diagnostics without provider data',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',config=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-smoke-model'}),result=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch:async()=>({ok:false,status:400,text:async()=>`raw ${secret}`})});assert.deepEqual(JSON.parse(JSON.stringify(result)),{ok:false,provider:'gemini',model:'gemini-smoke-model',stage:'transport',code:'provider-error',httpStatus:400,category:'request-schema-failure'});assert.equal(JSON.stringify(result).includes(secret),false)
});
