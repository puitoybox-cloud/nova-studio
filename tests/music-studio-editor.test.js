...
test('provider transport sends PR11 OpenAI payload and returns a PR12 result',async()=>{
  const core=load(),{request}=adapterFixture(core),input=core.createPartialEditAIInput(request),instruction=core.createPartialEditInstruction({intent:'brighter'}),context=core.createPartialEditPromptContext(request,input,instruction),payload=core.createPartialEditProviderPayload('openai',context),payloadSnapshot=JSON.stringify(payload),output={version:1,trackId:request.trackId,part:request.part,range:request.range,updates:[],adds:[],deletes:[]},calls=[];
  ...
  const body=JSON.parse(calls[0][1].body),schema=body.text.format.schema;assert.equal(calls[0][0],'https://api.openai.com/v1/responses');assert.equal(calls[0][1].headers.Authorization,'Bearer openai-test-secret');assert.deepEqual(Object.keys(body).sort(),['input','model','store','text']);assert.equal(JSON.stringify(body.input),JSON.stringify(payload.messages));assert.equal(body.text.format.type,'json_schema');assert.equal(body.text.format.name,'partial_edit_changes');assert.equal(body.text.format.strict,true);assert.equal(schema.additionalProperties,false);assert.equal(schema.properties.range.additionalProperties,false);assert.equal(schema.properties.updates.items.additionalProperties,false);assert.equal(schema.properties.adds.items.additionalProperties,false);assert.deepEqual(JSON.parse(JSON.stringify(schema.properties.version)),{type:'integer',enum:[1]});assert.equal(Object.hasOwn(schema.properties.version,'const'),false);assert.deepEqual(JSON.parse(JSON.stringify(schema.properties.part)),{type:'string',enum:['melody','drums','bass']});assert.equal(schema.properties.range.properties.startMeasure.minimum,1);assert.equal(schema.properties.range.properties.endMeasure.minimum,1);assert.equal(schema.properties.range.properties.startTick.minimum,0);assert.equal(schema.properties.range.properties.endTick.exclusiveMinimum,0);assert.equal(schema.properties.updates.items.properties.pitch.minimum,0);assert.equal(schema.properties.updates.items.properties.pitch.maximum,127);assert.equal(schema.properties.adds.items.properties.velocity.minimum,1);assert.equal(schema.properties.adds.items.properties.velocity.maximum,127);assert.equal(Object.hasOwn(payload.structuredOutput.schema,'additionalProperties'),false);assert.equal(Object.hasOwn(payload.structuredOutput.schema.properties.range,'additionalProperties'),false);assert.equal(Object.hasOwn(payload.structuredOutput.schema.properties.updates.items,'additionalProperties'),false);assert.equal(Object.hasOwn(payload.structuredOutput.schema.properties.adds.items,'additionalProperties'),false);assert.equal(payload.structuredOutput.schema.properties.version.const,1);assert.deepEqual(JSON.parse(JSON.stringify(payload.structuredOutput.schema.properties.part)),{enum:['melody','drums','bass']});assert.equal(JSON.stringify(payload),payloadSnapshot);assert.equal(body.store,false);assert.equal(result.status,'success');assert.deepEqual(JSON.parse(JSON.stringify(core.extractPartialEditProviderOutput(result))),JSON.parse(JSON.stringify(output)));assert.equal(JSON.stringify(result).includes('openai-test-secret'),false)
});
test('provider transport sends PR11 Gemini payload and returns a PR12 result',async()=>{
  ...
  const body=JSON.parse(calls[0][1].body),schema=body.generationConfig.responseFormat.text.schema;assert.equal(calls[0][0],'https://generativelanguage.googleapis.com/v1beta/models/gemini-test:generateContent');assert.equal(calls[0][1].headers['x-goog-api-key'],'gemini-test-secret');assert.equal(JSON.stringify(body.systemInstruction),JSON.stringify(payload.systemInstruction));assert.equal(JSON.stringify(body.contents),JSON.stringify(payload.contents));assert.equal(body.generationConfig.responseFormat.text.mimeType,'APPLICATION_JSON');assert.notEqual(body.generationConfig.responseFormat.text.mimeType,'application/json');assert.equal(Object.hasOwn(body.generationConfig,'responseMimeType'),false);assert.equal(Object.hasOwn(body.generationConfig,'responseSchema'),false);assert.deepEqual(schema.properties.version,{type:'integer'});assert.deepEqual(JSON.parse(JSON.stringify(schema.properties.part)),{enum:['melody','drums','bass'],type:'string'});assert.deepEqual([schema.type,schema.properties.version.type,schema.properties.trackId.type,schema.properties.range.properties.startTick.type,schema.properties.updates.type],['object','integer','string','number','array']);assert.equal(JSON.stringify(schema).includes('OBJECT'),false);assert.equal(JSON.stringify(schema).includes('INTEGER'),false);assert.equal(JSON.stringify(schema).includes('STRING'),false);assert.equal(Object.hasOwn(schema.properties.version,'const'),false);assert.equal(Object.hasOwn(schema.properties.range.properties.endTick,'exclusiveMinimum'),false);assert.equal(payload.generationConfig.responseSchema.properties.version.const,1);assert.deepEqual(JSON.parse(JSON.stringify(payload.generationConfig.responseSchema.properties.part)),{enum:['melody','drums','bass']});assert.equal(payload.generationConfig.responseSchema.properties.range.properties.endTick.exclusiveMinimum,0);assert.equal(result.status,'success');assert.deepEqual(JSON.parse(JSON.stringify(core.extractPartialEditProviderOutput(result))),JSON.parse(JSON.stringify(output)));assert.equal(JSON.stringify(result).includes('gemini-test-secret'),false)
});
...
test('OpenAI HTTP diagnostics preserve only validated allowlisted fields',async()=>{
  const core=load(),{request}=adapterFixture(core),input=core.createPartialEditAIInput(request),instruction=core.createPartialEditInstruction({intent:'darker'}),context=core.createPartialEditPromptContext(request,input,instruction),payload=core.createPartialEditProviderPayload('openai',context),secret='API_KEY_STYLE_SECRET_SHOULD_NEVER_LEAK_12345',execute=(body,requestId='req_safe-123')=>core.executePartialEditProviderRequest('openai',payload,{apiKey:secret,model:'openai-test',fetch:async()=>({ok:false,status:400,headers:{get:name=>name==='x-request-id'?requestId:null},text:async()=>typeof body==='string'?body:JSON.stringify(body)})});
  const valid=await execute({error:{type:'invalid_request_error',code:'invalid_json_schema',param:'text.format.schema',message:secret},authorization:secret,nested:{credential:secret}});assert.deepEqual(JSON.parse(JSON.stringify(valid.error)),{code:'provider-error',message:'The provider did not return a usable result.',httpStatus:400,category:'provider-error',providerType:'invalid_request_error',providerParam:'text.format.schema',requestId:'req_safe-123'});assert.equal(core.validatePartialEditProviderExecutionResult(valid).ok,true);assert.equal(JSON.stringify(valid).includes(secret),false);
  for(const [body,requestId]of [[{error:{type:'bad\nsecret',code:'https://secret.example/',param:'text.format.schema\nsecret',message:secret}},'request secret'],[{error:{type:{nested:secret},code:[secret],param:{credential:secret},message:secret}},'req_bad/secret'],[{error:{type:secret,code:secret,param:secret,message:secret}},'req_bad/secret'],['{bad','req_bad/secret']]){const result=await execute(body,requestId);assert.deepEqual(JSON.parse(JSON.stringify(result.error)),{code:'provider-error',message:'The provider did not return a usable result.',httpStatus:400,category:'provider-error'});assert.equal(core.validatePartialEditProviderExecutionResult(result).ok,true);assert.equal(JSON.stringify(result).includes(secret),false)}
  for(const [field,value]of [['providerType','server_error'],['providerParam','metadata.secret'],['requestId','request/value']]){const changed=JSON.parse(JSON.stringify(valid));changed.error[field]=value;assert.equal(core.validatePartialEditProviderExecutionResult(changed).ok,false)}
});
test('Gemini BadRequest diagnostics preserve only a validated provider field path',async()=>{
  const core=load(),{request}=adapterFixture(core),input=core.createPartialEditAIInput(request),instruction=core.createPartialEditInstruction({intent:'darker'}),context=core.createPartialEditPromptContext(request,input,instruction),payload=core.createPartialEditProviderPayload('gemini',context),secret='API_KEY_STYLE_SECRET_SHOULD_NEVER_LEAK_12345',field='generationConfig.responseFormat.text.schema.properties.version.type',execute=body=>core.executePartialEditProviderRequest('gemini',payload,{apiKey:'test-only-key',model:'gemini-test',fetch:async()=>({ok:false,status:400,text:async()=>typeof body==='string'?body:JSON.stringify(body)})}),detail=(type,violations)=>({error:{status:'INVALID_ARGUMENT',message:secret,details:[{'@type':type,fieldViolations:violations}]}});
  const valid=await execute(detail('type.googleapis.com/google.rpc.BadRequest',[{field,description:secret}]));assert.equal(valid.error.providerStatus,'INVALID_ARGUMENT');assert.equal(valid.error.category,'request-invalid');assert.equal(valid.error.providerField,field);assert.equal(core.validatePartialEditProviderExecutionResult(valid).ok,true);assert.equal(JSON.stringify(valid).includes(secret),false);
  for(const body of [detail('type.googleapis.com/google.rpc.BadRequest',[{field:'generationConfig\nSECRET',description:secret}]),detail('type.googleapis.com/google.rpc.BadRequest',[{field:'https://secret.example/',description:secret}]),detail('type.googleapis.com/google.rpc.ErrorInfo',[{field,description:secret}]),{error:{status:'INVALID_ARGUMENT',details:[{'@type':'type.googleapis.com/google.rpc.BadRequest'}]}},'{bad']){const result=await execute(body);assert.equal(Object.hasOwn(result.error,'providerField'),false);assert.equal(JSON.stringify(result).includes(secret),false)}
  for(const providerField of ['',null,'field path','field/path','field:secret','field[abc]','a'.repeat(201)]){const changed=JSON.parse(JSON.stringify(valid));changed.error.providerField=providerField;assert.equal(core.validatePartialEditProviderExecutionResult(changed).ok,false)}
});
...
test('live API smoke exposes sanitized transport diagnostics without provider data',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',providerField='generationConfig.responseFormat.text.schema.properties.version.type',config=core.createPartialEditProviderConfig('gemini',{apiKey:secret,model:'gemini-smoke-model'}),result=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch:async()=>({ok:false,status:400,text:async()=>JSON.stringify({error:{status:'INVALID_ARGUMENT',message:secret,details:[{'@type':'type.googleapis.com/google.rpc.BadRequest',fieldViolations:[{field:providerField,description:secret}]}]}})})});assert.deepEqual(JSON.parse(JSON.stringify(result)),{ok:false,provider:'gemini',model:'gemini-smoke-model',stage:'transport',code:'provider-error',httpStatus:400,providerStatus:'INVALID_ARGUMENT',category:'request-invalid',providerField});assert.equal(JSON.stringify(result).includes(secret),false)
});
test('live API smoke exposes only safe OpenAI HTTP diagnostics',async()=>{
  const core=load(),secret='NOVA_PR15_SECRET_DO_NOT_LEAK_12345',config=core.createPartialEditProviderConfig('openai',{apiKey:secret,model:'openai-smoke-model'}),result=await core.runPartialEditLiveApiSmokeTest({config,apiKey:secret},{fetch:async()=>({ok:false,status:400,headers:{get:name=>name==='x-request-id'?'req_smoke-123':null},text:async()=>JSON.stringify({error:{type:'invalid_request_error',code:secret,param:'text.format.schema',message:secret}})})});assert.deepEqual(JSON.parse(JSON.stringify(result)),{ok:false,provider:'openai',model:'openai-smoke-model',stage:'transport',code:'provider-error',httpStatus:400,category:'provider-error',providerType:'invalid_request_error',providerParam:'text.format.schema',requestId:'req_smoke-123'});assert.equal(JSON.stringify(result).includes(secret),false)
});
...
test('selected note length presets use PPQ ticks and one Undo Redo action without changing performance data',()=>{
  const core=load(),values={'1/1':4,'1/2':2,'1/4':1,'1/8':.5,'1/16':.25,'1/32':.125};
  for(const [length,beats] of Object.entries(values)){const session=core.createSession({midiData:{ppq:960,tracks:[{part:'melody',notes:[{id:'one',pitch:61,startTick:321,durationTicks:77,velocity:83},{id:'two',pitch:66,startTick:987,durationTicks:155,velocity:104}]}]}}),before=core.clone(core.currentTrack(session).notes);core.selectAllNotes(session);const result=core.setSelectedNoteLength(session,length);assert.deepEqual(JSON.parse(JSON.stringify(result)),{ok:true,changed:true,count:2,durationTicks:Math.round(960*beats)});assert.equal(JSON.stringify(Array.from(core.currentTrack(session).notes,note=>[note.startTick,note.pitch,note.velocity,note.durationTicks])),JSON.stringify(before.map(note=>[note.startTick,note.pitch,note.velocity,Math.round(960*beats)])));assert.equal(session.undo.length,1);assert.equal(session.dirty,true);core.undo(session);assert.equal(JSON.stringify(core.currentTrack(session).notes),JSON.stringify(before));core.redo(session);assert.deepEqual(Array.from(core.currentTrack(session).notes,note=>note.durationTicks),[Math.round(960*beats),Math.round(960*beats)])}
});

test('selected note length presets are no-op without editable selection and share all three track contracts',()=>{
  const core=load(),session=core.createSession({midiData:{ppq:480,tracks:[{part:'melody',notes:[{id:'m',pitch:60,startTick:0,durationTicks:90,velocity:80}]},{part:'drums',channel:10,notes:[{id:'d',pitch:38,startTick:120,durationTicks:60,velocity:110}]},{part:'bass',channel:2,notes:[{id:'b',pitch:36,startTick:240,durationTicks:180,velocity:90}]}]}});assert.equal(core.setSelectedNoteLength(session,'1/4').reason,'no-selection');assert.equal(session.undo.length,0);
  for(const part of ['melody','drums','bass']){core.selectPart(session,part);core.selectAllNotes(session);const note=core.clone(core.selectedNotes(session)[0]),result=core.setSelectedNoteLength(session,'1/8');assert.equal(result.durationTicks,240);const changed=core.selectedNotes(session)[0];assert.deepEqual([changed.startTick,changed.pitch,changed.velocity,changed.durationTicks],[note.startTick,note.pitch,note.velocity,240])}
});
...
test('External melodic Correction is exact-Track-ID scoped and preserves Track identity metadata and locked notes',()=>{
  const core=load(),session=core.createSession({projectId:'external-correction',midiData:{ppq:480,timeSignature:{numerator:4,denominator:4},editor:{measureCount:4},tracks:[
    {id:'melody',part:'melody',name:'Shared Name',channel:1,program:0,notes:[{id:'shared-note',pitch:61,startTick:119,durationTicks:251,velocity:70}]},
    {id:'drums',part:'drums',channel:10,notes:[]},{id:'bass',part:'bass',channel:2,program:32,notes:[]},
    {id:'external-a',name:'Shared Name',trackType:'midi-melodic',roleAssignment:'melody',channel:3,program:48,custom:{keep:true},notes:[{id:'shared-note',pitch:61,startTick:119,durationTicks:251,velocity:91,meta:'target'},{id:'locked',pitch:66,startTick:481,durationTicks:239,velocity:82,locked:true,meta:'keep'}]},
    {id:'external-b',name:'Shared Name',trackType:'midi-melodic',roleAssignment:'melody',channel:4,program:52,notes:[{id:'shared-note',pitch:63,startTick:121,durationTicks:249,velocity:92,meta:'other'}]}
  ]}}),before=core.clone(session.midiData.tracks),target=session.midiData.tracks.find(track=>track.id==='external-a');
  core.selectTrackById(session,'external-a');const context=core.createEditorContext(session),result=core.previewCorrection(session,{key:'C',scale:'Major',quantize:'1/16',strength:100,swing:0,target:'all',correctDuration:true,cleanShortOverlaps:true},true,true,context);
  assert.equal(result.ok,true);assert.equal(result.preview.targetTrackId,'external-a');assert.equal(JSON.stringify(target.notes),JSON.stringify(before.find(track=>track.id==='external-a').notes));
  core.applyCorrection(session,context);assert.deepEqual(JSON.parse(JSON.stringify(target.notes.map(note=>[note.id,note.pitch,note.startTick,note.durationTicks,note.velocity,note.meta]))),[['shared-note',60,120,240,91,'target'],['locked',66,481,239,82,'keep']]);
  for(const id of ['melody','external-b'])assert.equal(JSON.stringify(session.midiData.tracks.find(track=>track.id===id)),JSON.stringify(before.find(track=>track.id===id)));
  assert.deepEqual(JSON.parse(JSON.stringify({id:target.id,name:target.name,roleAssignment:target.roleAssignment,channel:target.channel,program:target.program,custom:target.custom})),{id:'external-a',name:'Shared Name',roleAssignment:'melody',channel:3,program:48,custom:{keep:true}});
  core.undo(session);assert.equal(JSON.stringify(session.midiData.tracks),JSON.stringify(before));core.redo(session);assert.equal(target.id,'external-a');assert.equal(core.currentTrack(session).notes[0].pitch,60)
});

test('External Correction rejects stale Track Preview stale selected-note state and unsupported External drums',()=>{
  const core=load(),session=core.createSession({projectId:'external-correction-stale',midiData:{tracks:[{part:'melody',notes:[]},{part:'drums',notes:[]},{part:'bass',notes:[]},{id:'external-a',trackType:'midi-melodic',roleAssignment:'unassigned',channel:3,notes:[{id:'same',pitch:61,startTick:119,durationTicks:251,velocity:90},{id:'second',pitch:63,startTick:361,durationTicks:239,velocity:80}]},{id:'external-b',trackType:'midi-melodic',roleAssignment:'unassigned',channel:4,notes:[{id:'same',pitch:65,startTick:121,durationTicks:249,velocity:91}]},{id:'external-drums',trackType:'midi-drums',roleAssignment:'drums',channel:10,notes:[{id:'kick',pitch:36,startTick:119,durationTicks:120,velocity:100}]}]}}),before=JSON.stringify(session.midiData.tracks);
  core.selectTrackById(session,'external-a');core.selectNote(session,'same');const context=core.createEditorContext(session),result=core.previewCorrection(session,{key:'C',scale:'Major',quantize:'1/16',strength:100,swing:0,target:'selected'},true,true,context);assert.equal(result.ok,true);
  core.selectNote(session,'second');let applied=core.applyCorrection(session,context);assert.equal(applied.ok,false);assert.ok(applied.errors.includes('stale-selection'));assert.equal(JSON.stringify(session.midiData.tracks),before);
  core.selectTrackById(session,'external-a');core.previewCorrection(session,{key:'C',scale:'Major',quantize:'1/16',strength:100,swing:0,target:'all'});const preview=session.correctionPreview;core.selectTrackById(session,'external-b');session.correctionPreview=preview;applied=core.applyCorrection(session);assert.equal(applied.ok,false);assert.ok(applied.errors.includes('stale-preview'));assert.equal(JSON.stringify(session.midiData.tracks),before);
  core.selectTrackById(session,'external-drums');assert.equal(core.resolveTrackCapabilities(core.currentTrack(session)).supportsPitchCorrection,false);assert.equal(core.previewCorrection(session,{target:'all'}).ok,false);assert.equal(JSON.stringify(session.midiData.tracks),before);
  const assignedDrums=session.midiData.tracks.find(track=>track.id==='external-b');assignedDrums.roleAssignment='drums';core.selectTrackById(session,'external-b');assert.equal(core.resolveTrackType(assignedDrums),'midi-melodic');assert.equal(core.resolveTrackCapabilities(assignedDrums).supportsPitchCorrection,false);assert.equal(core.previewCorrection(session,{target:'all'}).ok,false)
});
