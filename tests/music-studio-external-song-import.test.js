...
test('production entry cache keys select the Review-capable Editor assets',()=>{
  ...
  assert.match(index,/app\.js\?v=1\.5\.56/);
  for(const source of [host,standalone]){assert.match(source,/music-studio\.css\?v=1\.4\.129/);assert.match(source,/music-studio-editor\.js\?v=1\.4\.19/);assert.match(source,/music-studio\.js\?v=1\.4\.108/)}
  assert.match(host,/MusicStudioEditor\?\.ASSET_VERSION==='1\.4\.19'&&typeof window\.MusicStudioEditor\.externalReviewTracks==='function'/);
  assert.equal((entries.match(/music-studio-editor\.js\?v=1\.4\.19/g)||[]).length,2);assert.equal((entries.match(/music-studio\.js\?v=1\.4\.108/g)||[]).length,2);assert.doesNotMatch(studio,/music-studio-editor\.js\?v=/);
});

test('runtime markers expose only module versions and the Review API type',()=>{
  const{app,editor}=loadApp();assert.equal(editor.ASSET_VERSION,'1.4.19');assert.equal(app.ASSET_VERSION,'1.4.108');assert.equal(typeof editor.externalReviewTracks,'function');
  const marker={editor:editor.ASSET_VERSION,studio:app.ASSET_VERSION,reviewApi:typeof editor.externalReviewTracks};assert.deepEqual(JSON.parse(JSON.stringify(marker)),{editor:'1.4.19',studio:'1.4.108',reviewApi:'function'});
});
