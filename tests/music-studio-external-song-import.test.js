...
test('external Track Review shows identifiers summaries and explicit unassigned defaults without auto assignment',()=>{
  ...
  assert.match(html,/<dialog class="music-editor-popover music-external-track-review" id="externalTrackReviewDialog"/);assert.doesNotMatch(html,/<dialog class="music-editor-popover music-external-track-review"[^>]*\sopen(?:\s|>)/);assert.match(html,/aria-label="External Track Reviewを閉じる">× Close（閉じる）/);
...
test('Editor render mounts exactly three review rows for the real six-Track compatibility layout',()=>{
  ...
  assert.equal((html.match(/music-editor-popover music-external-track-review/g)||[]).length,1);assert.equal((html.match(/music-external-track-review-entry/g)||[]).length,1);assert.equal((html.match(/class="music-external-track-row"/g)||[]).length,3);
...
test('production entry cache keys select the Review-capable Editor assets',()=>{
  ...
  for(const source of [host,standalone]){assert.match(source,/music-studio\.css\?v=1\.4\.127/);assert.match(source,/music-studio-editor\.js\?v=1\.4\.18/)}
  assert.match(host,/music-studio\.js\?v=1\.4\.103/);assert.match(standalone,/music-studio\.js\?v=1\.4\.105/);
  assert.match(host,/MusicStudioEditor\?\.ASSET_VERSION==='1\.4\.18'&&typeof window\.MusicStudioEditor\.externalReviewTracks==='function'/);
  assert.equal((entries.match(/music-studio-editor\.js\?v=1\.4\.18/g)||[]).length,2);assert.equal((entries.match(/music-studio\.js\?v=1\.4\.10[35]/g)||[]).length,2);assert.doesNotMatch(studio,/music-studio-editor\.js\?v=/);
});

test('runtime markers expose only module versions and the Review API type',()=>{
  const{app,editor}=loadApp();assert.equal(editor.ASSET_VERSION,'1.4.18');assert.equal(app.ASSET_VERSION,'1.4.103');assert.equal(typeof editor.externalReviewTracks,'function');
  const marker={editor:editor.ASSET_VERSION,studio:app.ASSET_VERSION,reviewApi:typeof editor.externalReviewTracks};assert.deepEqual(JSON.parse(JSON.stringify(marker)),{editor:'1.4.18',studio:'1.4.103',reviewApi:'function'});
});
...
test('compact Review entry opens as a non-modal Music Studio popover without changing assignment data',()=>{
  ...
  const dialog={calls:0,classList:{contains:name=>name==='music-external-track-review'},show(){this.calls++},showModal(){throw new Error('Review must not use showModal')}};window.document={getElementById:id=>id==='externalTrackReviewDialog'?dialog:null};assert.equal(app.openHelp('externalTrackReviewDialog'),true);assert.equal(dialog.calls,1);assert.equal(JSON.stringify(project.midiData.tracks),before);
});
...
test('External Track Review CSS remains compact at desktop tablet and phone boundaries',()=>{
  const css=fs.readFileSync(path.join(__dirname,'..','music-studio.css'),'utf8');
  assert.match(css,/\.music-midi-editor-page \.music-external-track-review-entry\{[^}]*white-space:nowrap/);assert.doesNotMatch(css,/\.music-external-track-review::backdrop/);assert.match(css,/\.music-external-track-review\{position:fixed;z-index:85;top:76px;right:12px/);assert.match(css,/grid-template-columns:minmax\(0,1fr\) 150px/);assert.match(css,/@media\(max-width:600px\)\{[^}]*\.music-external-track-review-entry/);assert.match(css,/\.music-external-track-review\{width:calc\(100vw - 16px\);max-height:calc\(100dvh - 20px\)/);
});
