...
test('production entry cache keys select the Review-capable Editor assets',()=>{
  ...
  for(const source of [host,standalone]){assert.match(source,/music-studio\.css\?v=1\.4\.127/);assert.match(source,/music-studio-editor\.js\?v=1\.4\.18/);assert.match(source,/music-studio\.js\?v=1\.4\.103/)}
  assert.match(host,/MusicStudioEditor\?\.ASSET_VERSION==='1\.4\.18'&&typeof window\.MusicStudioEditor\.externalReviewTracks==='function'/);
  assert.equal((entries.match(/music-studio-editor\.js\?v=1\.4\.18/g)||[]).length,2);assert.equal((entries.match(/music-studio\.js\?v=1\.4\.103/g)||[]).length,2);assert.doesNotMatch(studio,/music-studio-editor\.js\?v=/);
});
