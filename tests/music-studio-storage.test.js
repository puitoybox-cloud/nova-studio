...
test('Track-ID keyed editor state survives save reload without changing Track identity or metadata',async()=>{
  const indexedDB=fakeIndexedDB(),{app,core}=load({indexedDB}),repo=app.indexedDbRepository(),project=richProject(app,'track-context-persistence');app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.state.midiEditor=core.createSession(project);const before=plain(app.state.midiEditor.midiData.tracks);core.selectTrackById(app.state.midiEditor,'strings');core.setSelectedMeasures(app.state.midiEditor,[2,4]);await app.saveMidiEditor({silent:true});const stored=await repo.get(project.projectId),reloaded=core.createSession(stored);core.selectTrackById(reloaded,'strings');
  assert.deepEqual(plain(stored.midiData.tracks),before);assert.deepEqual(plain(stored.midiData.tracks.map(track=>track.id)),before.map(track=>track.id));assert.deepEqual(Array.from(reloaded.selectedMeasures),[2,4]);assert.deepEqual(Array.from(stored.midiData.editor.trackStates.strings.selectedMeasures),[2,4]);assert.equal(stored.schemaVersion,'1.0');assert.equal(app.APP_VERSION,'1.4.0')
});
test('External Track Quantize survives IndexedDB save reload without changing schema or Track identity',async()=>{
  const indexedDB=fakeIndexedDB(),{app,core}=load({indexedDB}),repo=app.indexedDbRepository(),project=richProject(app,'external-quantize-persistence'),external=project.midiData.tracks.find(track=>track.id==='strings');external.notes[0].startTick=301;app.setRepository(repo);await repo.put(project);app.state.projects=[project];const session=core.createSession(project),trackIds=plain(session.midiData.tracks.map(track=>track.id));app.state.midiEditor=session;core.selectTrackById(session,'strings');core.selectNote(session,'strings-low');const result=core.quantizeSelectedStarts(session,'1/4',core.createEditorContext(session));assert.equal(result.targetTrackId,'strings');assert.equal(core.resolveEditorTrack(session,'strings').notes[0].startTick,0);await app.saveMidiEditor({silent:true});const stored=await repo.get(project.projectId),reloaded=core.createSession(stored);assert.equal(stored.midiData.tracks.find(track=>track.id==='strings').notes[0].startTick,0);assert.deepEqual(plain(stored.midiData.tracks.map(track=>track.id)),trackIds);assert.equal(stored.schemaVersion,'1.0');assert.equal(app.APP_VERSION,'1.4.0');assert.equal(core.resolveEditorTrack(reloaded,'strings').notes[0].id,'strings-low')
});
...
test('optional trackType survives save reload JSON and backup recovery without backfill or schema migration',async()=>{
  const indexedDB=fakeIndexedDB(),{app}=load({indexedDB}),repo=app.indexedDbRepository(),project=richProject(app,'track-type-round-trip'),external=project.midiData.tracks.find(track=>track.id==='strings');external.roleAssignment='strings';external.trackType='midi-melodic';app.setRepository(repo);await repo.put(project);app.state.projects=[project];
  const reloaded=await repo.get(project.projectId),exported=await app.exportProject(project.projectId),jsonTrack=JSON.parse(exported.text).midiData.tracks.find(track=>track.id==='strings'),backup=app.backupObject(),backupTrack=backup.projects[0].midiData.tracks.find(track=>track.id==='strings');
  for(const track of [reloaded.midiData.tracks.find(track=>track.id==='strings'),jsonTrack,backupTrack]){assert.equal(track.id,'strings');assert.equal(track.trackType,'midi-melodic');assert.equal(track.roleAssignment,'strings');assert.equal(track.notes.length,2)}
  const fresh=load(),target=fresh.app.memoryRepository();fresh.app.setRepository(target);assert.equal((await fresh.app.importText(exported.text)).ok,true);const imported=await target.get(project.projectId),importedTrack=imported.midiData.tracks.find(track=>track.id==='strings');assert.equal(importedTrack.trackType,'midi-melodic');assert.equal(importedTrack.roleAssignment,'strings');
  const recovery=load(),recoveryRepo=recovery.app.memoryRepository();recovery.app.setRepository(recoveryRepo);const restored=await recovery.app.restoreBackup(backup,{settings:false,projects:true});assert.equal(restored.ok,true);const recoveredTrack=(await recoveryRepo.get(project.projectId)).midiData.tracks.find(track=>track.id==='strings');assert.equal(recoveredTrack.trackType,'midi-melodic');assert.equal(recoveredTrack.roleAssignment,'strings');
  for(const track of project.midiData.tracks.filter(track=>track.id!=='strings'))assert.equal(Object.hasOwn(track,'trackType'),false);assert.equal(imported.schemaVersion,'1.0');assert.equal(app.APP_VERSION,'1.4.0')
});

test('tempo map survives IndexedDB JSON backup and editor reload without schema changes',async()=>{const indexedDB=fakeIndexedDB(),{app,core}=load({indexedDB}),repo=app.indexedDbRepository(),project=richProject(app,'tempo-map-persistence');project.midiData.tempoMap=[{tick:0,bpm:137,microsecondsPerQuarter:437956,track:0},{tick:3360,bpm:91.5,microsecondsPerQuarter:655738,track:0}];app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.state.midiEditor=core.createSession(project);core.updateDirty(app.state.midiEditor);app.state.midiEditor.dirty=true;await app.saveMidiEditor({silent:true});const reloaded=await repo.get(project.projectId),exported=await app.exportProject(project.projectId),backup=app.backupObject(),fresh=load(),target=fresh.app.memoryRepository();fresh.app.setRepository(target);const imported=await fresh.app.importText(exported.text),restored=await target.get(imported.project.projectId),session=fresh.core.createSession(restored);for(const value of [reloaded,restored,backup.projects[0],{midiData:session.midiData,schemaVersion:restored.schemaVersion}]){assert.equal(JSON.stringify(value.midiData.tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,137],[3360,91.5]]));assert.equal(value.schemaVersion,'1.0')}});
