...
test('Drums Web MIDI recording locks its target preserves GM hits and exports channel 10 without Program Change',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'record-drums',projectName:'Record Drums'}),input={id:'pads',name:'Drum Pads',onmidimessage:null};app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);app.editorSelectPart('drums');let clock=1000,frame=null;window.performance={now:()=>clock};window.requestAnimationFrame=callback=>{frame=callback;return 1};window.cancelAnimationFrame=()=>{frame=null};const previews=[];app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,previewTrackNote(...args){previews.push(args)},noteOn(){},noteOff(){},allNotesOff(){},stopPreview(){},stopPlayback(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'pads'});await app.editorSelectMidiInput('pads');await app.editorStartMidiRecording({skipCountIn:true});assert.equal(app.state.midiInput.recordingPart,'drums');app.editorSelectPart('melody');for(const [index,pitch] of [36,38,42,46].entries()){const start=1000+index*200;input.onmidimessage({data:[0x90,pitch,90+index],timeStamp:start});if(pitch!==46)input.onmidimessage({data:[0x80,pitch,0],timeStamp:start+120})}clock=1800;const result=await app.editorStopMidiRecording();assert.equal(result.targetPart,'drums');const stored=await repo.get(project.projectId),track=stored.midiData.tracks.find(item=>item.part==='drums');assert.deepEqual(Array.from(track.notes,note=>note.pitch),[36,38,42,46]);assert.deepEqual(Array.from(track.notes,note=>note.velocity),[90,91,92,93]);assert.ok(track.notes.every(note=>note.durationTicks>0));assert.ok(track.notes.every(note=>note.locked===false));assert.equal(track.channel,10);assert.equal(track.program,null);assert.deepEqual(previews.map(args=>args.slice(0,2)),[['drums',36],['drums',38],['drums',42],['drums',46]]);const parsed=window.MusicStudioMidiParser.parseMidiFile(window.MusicStudioMidi.createMidiFile(app.midiExportSummary(stored,'drums').input).bytes).normalized,exported=parsed.tracks.find(item=>item.noteCount);assert.equal(exported.channel,10);assert.equal(exported.programChanges.length,0);app.editorUndo();assert.equal(app.state.midiEditor.midiData.tracks.find(item=>item.part==='drums').notes.length,0);app.editorRedo();assert.equal(app.state.midiEditor.midiData.tracks.find(item=>item.part==='drums').notes.length,4);
});
test('Bass Web MIDI recording shares Count-in timing ignores early input and remains locked after a UI part change',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'record-bass',projectName:'Record Bass'}),input={id:'bass-keys',name:'Bass Keys',onmidimessage:null};app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);app.editorSelectPart('bass');let clock=1000;window.performance={now:()=>clock};window.setTimeout=()=>1;window.clearTimeout=()=>{};window.requestAnimationFrame=()=>1;window.cancelAnimationFrame=()=>{};const previews=[];app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,previewTrackNote(...args){previews.push(args)},noteOn(){},noteOff(){},allNotesOff(){},stopPreview(){},stopPlayback(){},metronomeClick(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'bass-keys'});await app.editorSelectMidiInput('bass-keys');const pending=app.editorStartMidiRecording();await new Promise(resolve=>setImmediate(resolve));const boundary=app.state.midiInput.recordingScheduledAt;input.onmidimessage({data:[0x90,24,70],timeStamp:boundary-10});input.onmidimessage({data:[0x80,24,0],timeStamp:boundary-5});clock=boundary;input.onmidimessage({data:[0x90,31,104],timeStamp:boundary});await pending;app.editorSelectPart('drums');input.onmidimessage({data:[0x80,31,0],timeStamp:boundary+375});clock=boundary+400;await app.editorStopTransport();const stored=await repo.get(project.projectId),bass=stored.midiData.tracks.find(item=>item.part==='bass');assert.equal(bass.notes.length,1);assert.equal(bass.notes[0].locked,false);assert.deepEqual([bass.notes[0].pitch,bass.notes[0].velocity,bass.notes[0].startTick,bass.notes[0].durationTicks],[31,104,0,360]);assert.equal(bass.channel,2);assert.equal(bass.program,32);assert.equal(stored.midiData.tracks.find(item=>item.part==='drums').notes.length,0);assert.deepEqual(previews.map(args=>args[0]),['bass','bass']);const parsed=window.MusicStudioMidiParser.parseMidiFile(window.MusicStudioMidi.createMidiFile(app.midiExportSummary(stored,'bass').input).bytes).normalized,exported=parsed.tracks.find(item=>item.noteCount);assert.equal(exported.channel,2);assert.equal(exported.programChanges[0].program,32);
});
test('Drums recording also excludes Count-in hits from its fixed target',async()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'drums-count-in',projectName:'Drums Count-in'}),input={id:'pads',name:'Pads',onmidimessage:null};app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);app.editorSelectPart('drums');let clock=500;window.performance={now:()=>clock};window.setTimeout=()=>1;window.clearTimeout=()=>{};window.requestAnimationFrame=()=>1;window.cancelAnimationFrame=()=>{};app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,previewTrackNote(){},allNotesOff(){},stopPreview(){},stopPlayback(){},metronomeClick(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'pads'});await app.editorSelectMidiInput('pads');const pending=app.editorStartMidiRecording();await new Promise(resolve=>setImmediate(resolve));const boundary=app.state.midiInput.recordingScheduledAt;input.onmidimessage({data:[0x90,35,80],timeStamp:boundary-20});input.onmidimessage({data:[0x80,35,0],timeStamp:boundary-10});clock=boundary;input.onmidimessage({data:[0x90,36,99],timeStamp:boundary});await pending;input.onmidimessage({data:[0x80,36,0],timeStamp:boundary+125});clock=boundary+130;await app.editorStopMidiRecording();const notes=app.state.midiEditor.midiData.tracks.find(track=>track.part==='drums').notes;assert.deepEqual(Array.from(notes,note=>[note.pitch,note.velocity]),[[36,99]]);
});
test('Record Stop and MIDI input controls remain available for Melody Drums and Bass without adding part Play controls',()=>{
  const{app}=load({requestMIDIAccess:async()=>({inputs:new Map()})}),project=app.makeProject({projectId:'record-controls-all-parts',projectName:'Record controls'});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);for(const part of ['melody','drums','bass']){app.editorSelectPart(part);const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/Record（録音）/);assert.match(html,/Stop（停止）/);assert.match(html,/aria-label="MIDI入力"/);if(part==='melody')assert.match(html,/Play（再生）/);else assert.doesNotMatch(html,/Play（再生）/)}
});
