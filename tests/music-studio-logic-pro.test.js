...
test('MIDI Input Assignment is runtime-only independent from device and visible Track',async()=>{
  ...
  app.state.midiInput.recording=true;app.state.midiInput.recordingPart='bass';app.state.midiInput.recordingTrackId='bass';assert.equal(app.editorSelectMidiTarget('melody'),false);assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/aria-label="Input To" disabled/);assert.equal(app.state.midiInput.recordingPart,'bass');assert.equal(app.state.midiInput.recordingTrackId,'bass');app.state.midiInput.recording=false;
});
test('unsupported Drums and Bass assignment stops before recording without mutating Melody',async()=>{
  const{app}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'unsupported-input-target',projectName:'Unsupported target',midiData:{tracks:[{part:'melody',notes:[{id:'kept',pitch:60,startTick:0,durationTicks:480,velocity:90}]}]}});app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);const before=JSON.stringify(app.state.midiEditor.midiData);
  for(const part of ['drums','bass']){assert.equal(app.editorSelectMidiTarget(part),true);const result=await app.editorStartMidiRecording({skipCountIn:true});assert.deepEqual(JSON.parse(JSON.stringify(result)),{ok:false,reason:'unsupported-recording-target',targetPart:part});assert.equal(app.state.midiInput.recording,false);assert.equal(app.state.midiInput.recordingPart,null);assert.equal(app.state.midiInput.recordingTrackId,null);assert.equal(JSON.stringify(app.state.midiEditor.midiData),before);assert.equal(app.state.midiEditor.dirty,false)}
  assert.equal(JSON.stringify((await repo.get(project.projectId)).midiData),JSON.stringify(project.midiData));
});
