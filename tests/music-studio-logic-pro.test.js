...
test('recording previews noteOn chords growth noteOff and Stop without duplicate committed notes',async()=>{
  ...
  const liveElements=new Map(),inserted=[];const roll={querySelector:()=>null,insertAdjacentHTML(_position,markup){inserted.push(markup);const id=markup.match(/data-live-note-id="([^"]+)/)?.[1];if(id)liveElements.set(id,{style:{}})}};window.document={body:{dataset:{}},querySelector(selector){if(selector==='.music-piano-roll')return roll;const id=selector.match(/data-live-note-id="([^"]+)/)?.[1];return id?liveElements.get(id)||null:null},addEventListener(){}};
...
test('Melody Editor visual polish keeps semantic controls while styling piano keys and workspace surfaces',()=>{
  ...
  assert.match(css,/button\.is-white::before\{z-index:1;top:1px;bottom:1px;left:0;width:calc\(100% - 8px\);height:auto;border:0;border-right:1px/);
  assert.match(css,/button\.is-white::before,[^}]*button\.is-white\.key-d::before[^}]*button\.is-white\.key-g::before[^}]*button\.is-white\.key-a::before[^}]*button\.is-white\.key-b::before\{top:1px;bottom:1px;height:auto;transform:none\}/);
