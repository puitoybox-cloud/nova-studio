...
test('Project and Editor BPM inputs display at most three decimals without changing stored tempo',()=>{
  const{app}=load(),cases=[[86.13301521539714,'86.133'],[120,'120'],[86.1,'86.1']];for(const[value,display]of cases){const project=app.makeProject({projectId:`bpm-display-${display}`,projectName:'BPM Display',bpm:value,midiData:{tempo:value,tracks:[{part:'melody',notes:[]}]}});app.state.projects=[project];app.state.midiEditor=null;const projectHtml=app.renderRoute(`music-studio/project/${project.projectId}`),editorHtml=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(projectHtml,new RegExp(`name="bpm"[^>]*step="0\\.001"[^>]*value="${display.replace('.','\\.')}"`));assert.match(editorHtml,new RegExp(`step="0\\.001"[^>]*value="${display.replace('.','\\.')}"[^>]*aria-label="Current Tempo BPM"`));assert.equal(project.musicalSettings.bpm,value);assert.equal(app.state.midiEditor.midiData.tempo,value)}
});
test('Tempo Change BPM input shares display formatting without mutating tempo or tempo map',()=>{
  const{app}=load(),cases=[[86.13301521539714,'86.133'],[120,'120'],[86.1,'86.1']];for(const[value,display]of cases){const project=app.makeProject({projectId:`tempo-change-display-${display}`,projectName:'Tempo Change Display',bpm:value,midiData:{tempo:value,tempoMap:[{tick:0,bpm:value,microsecondsPerQuarter:Math.round(60000000/value),track:0}],tracks:[{part:'melody',notes:[]}]}}),before=JSON.stringify(project);app.state.projects=[project];app.state.midiEditor=null;const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,new RegExp(`name="tempoChangeBpm"[^>]*step="0\\.001"[^>]*value="${display.replace('.','\\.')}"`));assert.equal(JSON.stringify(project),before);assert.equal(app.state.midiEditor.midiData.tempo,value);assert.equal(app.state.midiEditor.midiData.tempoMap[0].bpm,value)}
});
test('Tempo Change keeps decimal Preview Apply Remove and Clear behavior',()=>{
  const{app}=load(),exact=86.13301521539714,project=app.makeProject({projectId:'tempo-change-decimal-actions',projectName:'Tempo Change Decimal Actions',bpm:exact,midiData:{tempo:exact,editor:{measureCount:4},tracks:[{part:'melody',notes:[]}]}});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);const session=app.state.midiEditor;session.tempoChangeDraft={measure:2,bpm:90.125};assert.equal(app.editorPreviewTempoChange().preview.bpm,90.125);assert.equal(JSON.stringify(app.editorApplyTempoChange().tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,exact],[1920,90.125]]));assert.equal(app.editorRemoveTempoChange().changed,true);assert.equal(JSON.stringify(session.midiData.tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,exact]]));session.tempoChangeDraft={measure:3,bpm:91.375};app.editorApplyTempoChange();assert.equal(app.editorClearTempoChanges().changed,true);assert.equal(JSON.stringify(session.midiData.tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,exact]]));assert.equal(session.midiData.tempo,exact);assert.equal(app.state.projects[0].musicalSettings.bpm,exact)
});
test('BPM display formatting preserves exact persistence playback notes and manual edits across reopen',async()=>{
  const{app}=load(),repo=app.memoryRepository(),exact=86.13301521539714,tracks=[{id:'melody',part:'melody',notes:[{id:'melody-note',pitch:60,startTick:0,durationTicks:480,velocity:90}]},{id:'external',name:'External',roleAssignment:'unassigned',notes:[{id:'external-note',pitch:72,startTick:960,durationTicks:240,velocity:80}]}],project=app.makeProject({projectId:'bpm-display-persistence',projectName:'BPM Display Persistence',bpm:exact,midiData:{ppq:480,tempo:exact,tempoMap:[{tick:0,bpm:exact,microsecondsPerQuarter:696597,track:1}],tracks}});app.setRepository(repo);await repo.put(project);app.state.projects=[project];const before=JSON.stringify(project.midiData),html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`),session=app.state.midiEditor,normalizedTracks=JSON.stringify(session.midiData.tracks);assert.match(html,/value="86\.133"[^>]*aria-label="Current Tempo BPM"/);assert.equal(session.midiData.tempo,exact);assert.equal(session.midiData.tracks.find(track=>track.id==='external').notes[0].pitch,72);assert.equal(session.midiData.ppq,480);let playbackTempo=null;app.state.melodyAudio.synth={context:{currentTime:10},supported:()=>true,unlock:async()=>true,stopPlayback(){},stopMetronome(){},playTracks(_tracks,timing){playbackTempo=timing.tempo;return{ok:true,noteCount:1,durationMs:1000,playbackStart:10.04,secondsPerTick:60/(timing.tempo*timing.ppq),startTick:0,endTick:480,tempoMap:timing.tempoMap}}};await app.editorPlayMelody();assert.equal(playbackTempo,exact);app.editorStopMelody();assert.equal(JSON.stringify(project.midiData),before);assert.equal((await repo.get(project.projectId)).midiData.tempo,exact);assert.equal(app.editorSetBpm('91.25').ok,true);await app.state.midiEditorSavePromise;const stored=await repo.get(project.projectId);assert.equal(stored.musicalSettings.bpm,91.25);assert.equal(stored.midiData.tempo,91.25);assert.equal(JSON.stringify(stored.midiData.tracks),normalizedTracks);assert.equal(stored.midiData.ppq,480);app.state.midiEditor=null;app.state.projects=[stored];const reopenedProject=app.renderRoute(`music-studio/project/${project.projectId}`),reopenedEditor=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(reopenedProject,/name="bpm"[^>]*value="91\.25"/);assert.match(reopenedEditor,/value="91\.25"[^>]*aria-label="Current Tempo BPM"/);assert.equal(app.state.midiEditor.midiData.tempo,91.25)
});
...
test('Melody Correction is an overlay with complete basic controls and transient Preview Apply Cancel history',()=>{
  ...
  const css=fs.readFileSync(path.join(__dirname,'..','music-studio.css'),'utf8');assert.match(css,/\.music-midi-editor-page \.music-correction-popover label\{font-weight:700\}/);assert.match(css,/\.music-midi-editor-page \.music-correction-grid label[^}]*color:var\(--music-violet\)/);assert.match(css,/\.music-midi-editor-page \.music-correction-target legend\{margin-left:0;padding:0 5px;background:#081421;color:var\(--music-blue\);line-height:1\.4\}/);assert.match(css,/\.music-correction-batch-grid label\{color:var\(--music-ink\)\}/);assert.match(css,/\.music-correction-batch-grid :is\(b,legend,\.music-correction-group-title\)\{color:var\(--music-violet\)\}/);assert.doesNotMatch(css,/\.music-correction-batch-grid[^}]*color:#082032/);assert.doesNotMatch(css,/label:has\(:disabled\)/);
  assert.match(css,/input\[type=range\],input\[type=checkbox\],input\[type=radio\]\)\{accent-color:var\(--music-editor-accent\)\}/);assert.match(css,/:is\(\.music-correction-popover,\.music-cleanup-popover\) output\{color:var\(--music-violet\)\}/);assert.match(css,/:is\(\.music-correction-popover,\.music-cleanup-popover\) \.music-primary\{border-color:var\(--music-editor-accent\);background:rgba\(109,40,217,\.2\);color:var\(--music-violet\)\}/);assert.match(css,/legend\{float:none;grid-column:1\/-1;width:auto;margin:0 0 4px;padding:0;background:transparent;font-size:\.76rem;line-height:1\.25\}/);assert.match(css,/:is\(\.music-correction-popover,\.music-cleanup-popover\) button\{height:auto;min-height:28px;padding:3px 8px;font-size:\.74rem/);
...
test('Core and exact-ID External Melody render capability-gated Correction with working Close and reopen contracts',()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'external-batch-shell',projectName:'External batch shell',midiData:{tracks:[
    {id:'melody',part:'melody',name:'Melody',notes:[{id:'same-note',pitch:60,startTick:0,durationTicks:480,velocity:90}]},
    {id:'external-duplicate',name:'Melody',trackType:'midi-melodic',roleAssignment:'melody',notes:[{id:'same-note',pitch:72,startTick:0,durationTicks:240,velocity:80}]}
  ]}});app.state.projects=[project];const route=`music-studio/midi-editor/${project.projectId}`;
  let html=app.renderRoute(route);assert.match(html,/<summary>Melody Correction（メロディ補正）<\/summary>/);for(const selector of ['melodyCorrectionForm','melodyTransposePanel','melodyNoteLengthPanel'])assert.match(html,new RegExp(`id="${selector}"`));
  assert.equal(window.MusicStudioEditor.selectTrackById(app.state.midiEditor,'external-duplicate'),true);html=app.renderRoute(route);assert.equal(app.state.midiEditor.selectedTrackId,'external-duplicate');assert.match(html,/<summary>Melody Correction（メロディ補正）<\/summary>/);for(const selector of ['melodyCorrectionForm','melodyTransposePanel','melodyNoteLengthPanel'])assert.match(html,new RegExp(`id="${selector}"`));assert.doesNotMatch(html,/music-tempo-panel/);assert.equal((html.match(/music-correction-panel-close/g)||[]).length,1);assert.match(html,/External Melody Correctionパネル/);assert.match(html,/music-correction-panel-close" onclick="MusicStudio.editorClosePopup\(this\)"/);
...
test('editor chrome is compact, Melody helpers stay intact, and Correction uses a responsive overlay',()=>{
  ...
  assert.match(html,/class="music-correction-sticky-header"><button type="button" class="music-secondary music-correction-panel-close"[^>]*editorClosePopup\(this\)/);assert.equal((html.match(/music-correction-panel-close/g)||[]).length,1);
...
test('Piano Roll shortcuts share button actions and never fire from an input',async()=>{
  ...
  for(const selector of ['input','textarea','select','[contenteditable]']){const blocked=event(' ',{target:{closest:query=>query.includes(selector)?{}:null}});assert.equal(app.editorHandleShortcut(blocked),false);assert.equal(blocked.prevented,false)}
  for(const blocked of [event(' ',{isComposing:true}),event(' ',{repeat:true})]){assert.equal(app.editorHandleShortcut(blocked),false);assert.equal(blocked.prevented,false)}
...
test('playback target selection accepts one or multiple canonical tracks without changing their order',()=>{
  ...
  assert.deepEqual(Array.from(app.editorPlaybackTracks(session),track=>track.part),['melody','drums','bass']);
...
test('Play sends Melody Drums and Bass through one shared transport scheduler and keeps playhead across parts',async()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'three-track-playback',projectName:'Three Track Playback',bpm:120,midiData:{ppq:480,tempo:120,editor:{measureCount:4,loopEnabled:true,loopStart:480,loopEnd:960,transport:{metronomeEnabled:false}},tracks:[
    {id:'melody',part:'melody',channel:1,program:0,notes:[{id:'m',pitch:60,startTick:480,durationTicks:240,velocity:90}]},
    {id:'drums',part:'drums',channel:10,program:null,notes:[{id:'d',pitch:38,startTick:480,durationTicks:120,velocity:110}]},
    {id:'bass',part:'bass',channel:2,program:32,notes:[{id:'b',pitch:36,startTick:480,durationTicks:480,velocity:84}]}
  ]}});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);const session=app.state.midiEditor,before=JSON.stringify(session.midiData.tracks),calls=[];let frame=null,stopCalls=0,currentTime=10;window.requestAnimationFrame=callback=>{frame=callback;return 1};window.cancelAnimationFrame=()=>{frame=null};app.state.melodyAudio.synth={context:{get currentTime(){return currentTime}},supported:()=>true,unlock:async()=>true,stopPlayback(){stopCalls++},playTracks(tracks,timing){calls.push({tracks:JSON.parse(JSON.stringify(tracks)),timing:{...timing}});return{ok:true,trackCount:tracks.length,noteCount:tracks.reduce((count,track)=>count+track.notes.length,0),durationMs:630,playbackStart:10,secondsPerTick:.001,startTick:timing.startTick,endTick:timing.endTick}},stopMetronome(){}};
  session.playheadTick=0;const result=await app.editorPlayMelody();assert.equal(result.ok,true);assert.equal(calls.length,1);assert.deepEqual(calls[0].tracks.map(track=>[track.id,track.part,track.channel,track.program]),[['melody','melody',1,0],['drums','drums',10,null],['bass','bass',2,32]]);assert.equal(calls[0].timing.startTick,480);assert.equal(calls[0].timing.endTick,960);assert.deepEqual(JSON.parse(JSON.stringify(app.state.melodyAudio.transport)),{playing:true,currentTick:480,ppq:480,bpm:120,tempoMap:[{tick:0,bpm:120,microsecondsPerQuarter:500000,track:0}],loopEnabled:true,loopStart:480,loopEnd:960,playbackStartOrigin:10,activePlaybackTrackIds:['melody','drums','bass']});
  currentTime=10.24;frame();assert.ok(Math.abs(session.playheadTick-720)<1e-9);app.editorSelectPart('drums');assert.ok(Math.abs(session.playheadTick-720)<1e-9);app.editorSelectPart('bass');assert.ok(Math.abs(session.playheadTick-720)<1e-9);await app.editorStopTransport();assert.equal(app.state.melodyAudio.playing,false);assert.equal(app.state.melodyAudio.transport.playing,false);assert.ok(Math.abs(app.state.melodyAudio.transport.currentTick-720)<1e-9);assert.ok(stopCalls>=2);assert.equal(JSON.stringify(session.midiData.tracks),before);assert.equal(session.undo.length,0);assert.equal(session.dirty,false);
});
test('shared Play applies Mute runtime Solo ignores persisted-looking Solo and tolerates empty tracks',async()=>{
  const{app}=load(),project=app.makeProject({projectId:'playback-selection',projectName:'Playback Selection',midiData:{tracks:[
    {id:'melody',part:'melody',channel:1,program:0,solo:true,notes:[]},
    {id:'drums',part:'drums',channel:10,program:null,muted:true,notes:[{id:'d',pitch:36,startTick:0,durationTicks:120,velocity:100}]},
    {id:'bass',part:'bass',channel:2,program:32,notes:[{id:'b',pitch:36,startTick:0,durationTicks:480,velocity:80}]}
  ]}});app.state.projects=[project];const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`),before=JSON.stringify(app.state.midiEditor.midiData.tracks),calls=[],playButton=html.match(/<button class="music-primary" onclick="MusicStudio\.editorToggleMelodyPlayback\(\)"[^>]*>/)?.[0]||'';assert.ok(playButton);assert.doesNotMatch(playButton,/disabled/);app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,stopPlayback(){},playTracks(tracks,timing){calls.push({tracks:JSON.parse(JSON.stringify(tracks)),timing});return{ok:true,trackCount:tracks.length,noteCount:tracks.reduce((count,track)=>count+track.notes.length,0),durationMs:1000,playbackStart:0,secondsPerTick:.001,startTick:timing.startTick,endTick:480}},stopMetronome(){}};
  await app.editorPlayMelody();assert.deepEqual(calls[0].tracks.map(track=>track.id),['melody','bass']);app.editorStopMelody();app.state.melodyAudio.playbackState.soloByTrackId={bass:true};await app.editorPlayMelody();assert.deepEqual(calls[1].tracks.map(track=>track.id),['bass']);assert.equal(JSON.stringify(app.state.midiEditor.midiData.tracks),before);
});
test('three-track Mute controls persist through the existing track field while Solo stays runtime-only',async()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'mute-solo-controls',projectName:'Mute Solo Controls',midiData:{tracks:[
    {id:'melody',part:'melody',channel:1,program:0,notes:[{id:'m',pitch:60,startTick:0,durationTicks:480,velocity:90}]},
    {id:'drums',part:'drums',channel:10,program:null,notes:[{id:'d',pitch:36,startTick:0,durationTicks:120,velocity:100}]},
    {id:'bass',part:'bass',channel:2,program:32,notes:[{id:'b',pitch:36,startTick:0,durationTicks:480,velocity:80}]}
  ]}});app.state.projects=[project];app.setRepository(app.memoryRepository([project]));let html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`),session=app.state.midiEditor,core=window.MusicStudioEditor;
  for(const label of ['Melody','Drums','Bass']){assert.match(html,new RegExp(`aria-label="${label} Mute"`));assert.match(html,new RegExp(`aria-label="${label} Solo"`))}
  for(const part of ['melody','drums','bass']){app.editorSetTrackMuted(part,true);assert.equal(session.midiData.tracks.find(track=>track.part===part).muted,true);assert.deepEqual(Array.from(window.MusicStudioPlayback.activePlaybackTracks(session.midiData.tracks,app.state.melodyAudio.playbackState),track=>track.id),Array.from(session.midiData.tracks.filter(track=>!track.muted),track=>track.id))}
  assert.equal(session.undo.length,3);assert.equal(session.dirty,true);core.undo(session);assert.equal(session.midiData.tracks.find(track=>track.part==='bass').muted,false);
  for(const part of ['melody','drums'])app.editorSetTrackMuted(part,false);const projectBefore=JSON.stringify(session.midiData),undoBefore=session.undo.length,dirtyBefore=session.dirty;
  app.editorSetTrackSolo('bass',true);assert.deepEqual(Array.from(window.MusicStudioPlayback.activePlaybackTracks(session.midiData.tracks,app.state.melodyAudio.playbackState),track=>track.id),['bass']);app.editorSetTrackSolo('drums',true);assert.deepEqual(Array.from(window.MusicStudioPlayback.activePlaybackTracks(session.midiData.tracks,app.state.melodyAudio.playbackState),track=>track.id),['drums','bass']);
  app.editorSetTrackMuted('bass',true);assert.deepEqual(Array.from(window.MusicStudioPlayback.activePlaybackTracks(session.midiData.tracks,app.state.melodyAudio.playbackState),track=>track.id),['drums']);app.editorSetTrackSolo('bass',false);app.editorSetTrackSolo('drums',false);assert.deepEqual(Array.from(window.MusicStudioPlayback.activePlaybackTracks(session.midiData.tracks,app.state.melodyAudio.playbackState),track=>track.id),['melody','drums']);
  assert.equal(session.undo.length,undoBefore+1);core.undo(session);assert.equal(session.undo.length,undoBefore);assert.equal(JSON.stringify(session.midiData),projectBefore);assert.equal(session.dirty,dirtyBefore);assert.equal(Object.hasOwn(session.midiData.tracks[2],'solo'),false);
  app.editorSelectPart('drums');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/data-track-id="bass"[^]*aria-label="Bass Mute"[^]*aria-pressed="false"/);
});
...
test('MIDI target is runtime-only and follows the visible Track without an Input To control',async()=>{
  const{app}=load({requestMIDIAccess:async()=>({inputs:new Map()})}),project=app.makeProject({projectId:'input-assignment',projectName:'Input Assignment'});app.state.projects=[project];let html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`),session=app.state.midiEditor,beforeProject=JSON.stringify(project),beforeMidi=JSON.stringify(session.midiData),beforeUndo=session.undo.length,beforeRedo=session.redo.length;
  assert.equal(app.state.midiInput.targetPart,'melody');assert.doesNotMatch(html,/Input To|music-midi-target-select|editorSelectMidiTarget\(this\.value\)/);assert.match(html,/aria-label="MIDI機器選択"/);
  app.state.midiInput.selectedId='device-a';assert.equal(app.editorSelectMidiTarget('drums'),true);assert.equal(app.state.midiInput.targetPart,'drums');assert.equal(app.state.midiInput.selectedId,'device-a');for(const part of ['bass','drums','melody']){app.editorSelectPart(part);assert.equal(app.state.midiEditor.part,part);assert.equal(app.state.midiInput.targetPart,part);assert.equal(app.state.midiInput.selectedId,'device-a');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.doesNotMatch(html,/Input To|music-midi-target-select/);assert.match(html,/aria-label="MIDI機器選択"/)}assert.equal(app.editorSelectMidiTarget('bass'),true);assert.equal(app.state.midiInput.targetPart,'bass');
  assert.equal(JSON.stringify(project),beforeProject);assert.equal(JSON.stringify(session.midiData),beforeMidi);assert.equal(session.undo.length,beforeUndo);assert.equal(session.redo.length,beforeRedo);assert.equal(session.dirty,false);assert.doesNotMatch(JSON.stringify(app.backupObject()),/targetPart/);assert.doesNotMatch(JSON.stringify(app.getSettings()),/targetPart/);
  app.state.midiInput.recording=true;app.state.midiInput.recordingPart='bass';app.state.midiInput.recordingTrackId='bass';assert.equal(app.editorSelectMidiTarget('melody'),false);app.editorSelectPart('drums');assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.doesNotMatch(html,/Input To|music-midi-target-select/);assert.match(html,/aria-label="MIDI機器選択"/);assert.equal(app.state.midiInput.recordingPart,'bass');assert.equal(app.state.midiInput.recordingTrackId,'bass');app.state.midiInput.recording=false;app.state.midiInput.countingIn=true;assert.equal(app.editorSelectMidiTarget('drums'),false);app.editorSelectPart('melody');assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.doesNotMatch(html,/Input To|music-midi-target-select/);app.state.midiInput.countingIn=false;
});
test('MIDI preview routes to idle targetPart and prefers recordingPart while recording',async()=>{
  const{app}=load(),project=app.makeProject({projectId:'preview-routing',projectName:'Preview routing'}),input={id:'keys',name:'Keys',onmidimessage:null},calls=[];app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,noteOn(...args){calls.push(['melody',...args])},noteOff(){},previewTrackNote(...args){calls.push(['track',...args])}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'keys'});await app.editorSelectMidiInput('keys');
  app.state.midiInput.targetPart='drums';input.onmidimessage({data:[0x90,36,101],timeStamp:100});app.state.midiInput.targetPart='bass';input.onmidimessage({data:[0x90,40,102],timeStamp:200});assert.deepEqual(calls.map(call=>call.slice(0,4)),[['track','drums',36,101],['track','bass',40,102]]);
  app.state.midiInput.recording=true;app.state.midiInput.recordingPart='drums';app.state.midiInput.targetPart='bass';input.onmidimessage({data:[0x90,38,103],timeStamp:300});assert.deepEqual(calls.at(-1).slice(0,4),['track','drums',38,103]);
});
test('Drums and Bass assignment record only their fixed target with timing velocity Undo and persistence',async()=>{
  for(const [part,pitch,velocity,duration]of[['drums',36,101,120],['bass',31,104,360]]){const{app,window}=load({requestMIDIAccess:async()=>({inputs:new Map()})}),repo=app.memoryRepository(),project=app.makeProject({projectId:`record-${part}`,projectName:`Record ${part}`,midiData:{tracks:[{part:'melody',notes:[{id:'kept',pitch:60,startTick:0,durationTicks:480,velocity:80}]}]}}),input={id:'keys',name:'Keys',onmidimessage:null};app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);let clock=1000;window.performance={now:()=>clock};window.requestAnimationFrame=()=>1;window.cancelAnimationFrame=()=>{};app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,previewTrackNote(){},allNotesOff(){},stopPreview(){},stopPlayback(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'keys'});await app.editorSelectMidiInput('keys');app.editorSelectPart(part);assert.equal(app.state.midiEditor.part,part);assert.equal(app.state.midiInput.targetPart,part);assert.doesNotMatch(app.renderRoute(`music-studio/midi-editor/${project.projectId}`),/Input To|music-midi-target-select/);await app.editorStartMidiRecording({skipCountIn:true});assert.equal(app.state.midiInput.recordingPart,part);assert.equal(app.state.midiInput.recordingTrackId,part);assert.equal(app.editorSelectMidiTarget('melody'),false);input.onmidimessage({data:[0x90,pitch,velocity],timeStamp:1000});input.onmidimessage({data:[0x80,pitch,0],timeStamp:1000+duration/0.96});clock=1000+duration/0.96;const stopping=app.editorStopMidiRecording();assert.equal(app.state.midiEditor.dirty,true);const result=await stopping,tracks=app.state.midiEditor.midiData.tracks,target=tracks.find(track=>track.part===part),note=target.notes[0],targetNotes=()=>app.state.midiEditor.midiData.tracks.find(track=>track.part===part).notes;assert.equal(result.targetPart,part);assert.equal(app.state.midiEditor.part,part);assert.equal(app.state.midiInput.targetPart,part);assert.deepEqual([note.pitch,note.velocity,note.startTick,note.durationTicks],[pitch,velocity,0,duration]);assert.equal(tracks.find(track=>track.part==='melody').notes.length,1);assert.equal(tracks.find(track=>track.part===(part==='drums'?'bass':'drums')).notes.length,0);assert.equal(app.state.midiInput.recordingPart,null);assert.equal(app.state.midiInput.recordingTrackId,null);assert.equal((await repo.get(project.projectId)).midiData.tracks.find(track=>track.part===part).notes.length,1);app.editorUndo();assert.equal(targetNotes().length,0);app.editorRedo();assert.equal(targetNotes().length,1);await app.state.midiEditorSavePromise;app.state.midiEditor=null;app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.equal(app.state.midiEditor.midiData.tracks.find(track=>track.part===part).notes.length,1)}
});
test('Track ID recording freezes an unknown destination and Preview uses the safe Melody audio fallback',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),unknown={id:'external-track-10',name:'External Track',channel:10,program:81,sourceType:'external-ai',vendor:{keep:true},notes:[]},project=app.makeProject({projectId:'track-id-recording',projectName:'Track ID recording',midiData:{tracks:[{id:'melody',part:'melody',notes:[]},{id:'drums',part:'drums',notes:[]},{id:'bass',part:'bass',notes:[]},unknown]}}),input={id:'keys',name:'Keys',onmidimessage:null},calls=[];
  app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);let clock=1000;window.performance={now:()=>clock};window.requestAnimationFrame=()=>1;window.cancelAnimationFrame=()=>{};app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,noteOn(...args){calls.push(['melody',...args]);return true},noteOff(...args){calls.push(['off',...args]);return true},previewTrackNote(...args){calls.push(['track',...args]);return true},allNotesOff(){},stopPreview(){},stopPlayback(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'keys'});await app.editorSelectMidiInput('keys');const core=window.MusicStudioEditor,session=app.state.midiEditor;assert.equal(core.selectTrackById(session,'external-track-10'),true);assert.equal(session.part,'melody');assert.equal(await app.editorPreviewPitch(65,92),true);assert.equal(calls.at(-1)[0],'melody');
  await app.editorStartMidiRecording({skipCountIn:true});assert.equal(app.state.midiInput.recordingTrackId,'external-track-10');assert.equal(app.state.midiInput.recordingPart,null);core.selectPart(session,'bass');input.onmidimessage({data:[0x99,38,103],timeStamp:1000});input.onmidimessage({data:[0x89,38,0],timeStamp:1125});assert.equal(calls.some(call=>call[0]==='track'&&call[1]==='drums'),false);clock=1125;const result=await app.editorStopMidiRecording(),track=core.getTrackById(session.midiData.tracks,'external-track-10');assert.equal(result.targetTrackId,'external-track-10');assert.equal(track.notes.length,1);assert.deepEqual([track.notes[0].pitch,track.notes[0].velocity,track.notes[0].startTick,track.notes[0].durationTicks],[38,103,0,120]);assert.deepEqual(JSON.parse(JSON.stringify({sourceType:track.sourceType,vendor:track.vendor,channel:track.channel,program:track.program})),{sourceType:'external-ai',vendor:{keep:true},channel:10,program:81});assert.equal(session.midiData.tracks.find(item=>item.part==='melody').notes.length,0);assert.equal(session.midiData.tracks.find(item=>item.part==='bass').notes.length,0);assert.equal(app.state.midiInput.recordingTrackId,null);assert.equal(app.state.midiInput.recordingPart,null);assert.equal((await repo.get(project.projectId)).midiData.tracks.find(item=>item.id==='external-track-10').notes.length,1)
});
test('invalid or missing selectedTrackId safely falls back to the existing core recording target',async()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'invalid-track-id-recording',projectName:'Invalid Track ID'}),input={id:'keys',name:'Keys',onmidimessage:null};app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);window.requestAnimationFrame=()=>1;window.cancelAnimationFrame=()=>{};app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,allNotesOff(){},stopPreview(){},stopPlayback(){},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'keys',targetPart:'bass'});await app.editorSelectMidiInput('keys');app.state.midiEditor.selectedTrackId='missing-track';await app.editorStartMidiRecording({skipCountIn:true});assert.equal(app.state.midiInput.recordingTrackId,'bass');assert.equal(app.state.midiInput.recordingPart,'bass');await app.editorStopMidiRecording()
  app.state.midiEditor.selectedTrackId=undefined;await app.editorStartMidiRecording({skipCountIn:true});assert.equal(app.state.midiInput.recordingTrackId,'bass');assert.equal(app.state.midiInput.recordingPart,'bass');await app.editorStopMidiRecording()
});
test('shared Play Record Stop Count-in and Metronome controls remain available for Melody Drums and Bass',()=>{
  const{app}=load({requestMIDIAccess:async()=>({inputs:new Map()})}),project=app.makeProject({projectId:'record-controls-all-parts',projectName:'Record controls'});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);for(const part of ['melody','drums','bass']){app.editorSelectPart(part);const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);for(const label of ['Record（録音）','Play（再生）','Stop（停止）','Count-in','Metronome'])assert.match(html,new RegExp(label));assert.match(html,/aria-label="MIDI入力"/)}
});
...
test('selected Note Length preset UI saves through the existing Project JSON and MIDI duration paths',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'length-preset-ui',projectName:'Length Preset UI',midiData:{ppq:480,tracks:[{part:'melody',notes:[{id:'note',pitch:64,startTick:123,durationTicks:75,velocity:91}]}]}});app.setRepository(repo);await repo.put(project);app.state.projects=[project];let html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);for(const value of ['1/1','1/2','1/4','1/8','1/16','1/32'])assert.ok(html.includes(`editorSetSelectedNoteLength('${value}')`));assert.match(html,/Note Length/);assert.match(html,/Match Length（長さを揃える）/);
  const session=app.state.midiEditor,core=window.MusicStudioEditor;core.selectNote(session,'note');const result=app.editorSetSelectedNoteLength('1/2');assert.equal(result.durationTicks,960);await app.state.midiEditorSavePromise;const stored=await repo.get(project.projectId);assert.deepEqual([stored.midiData.tracks.find(track=>track.part==='melody').notes[0].startTick,stored.midiData.tracks.find(track=>track.part==='melody').notes[0].durationTicks],[123,960]);const exported=JSON.parse((await app.exportProject(project.projectId)).text);assert.equal(exported.midiData.tracks.find(track=>track.part==='melody').notes[0].durationTicks,960);assert.equal(app.midiExportInput(stored,'all').tracks.find(track=>track.part==='melody').notes[0].durationTicks,960)
});
...
test('External Partial Edit resnapshots its exact Track request when Edit Range changes after Start',()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'external-partial-range',projectName:'External Partial Range',midiData:{editor:{measureCount:24,editRange:{startMeasure:1,endMeasure:1}},tracks:[{id:'melody',part:'melody',notes:[]},{id:'drums',part:'drums',notes:[]},{id:'bass',part:'bass',notes:[]},{id:'vocals',name:'Vocals',trackType:'midi-melodic',roleAssignment:'unassigned',notes:[{id:'imported-note',pitch:72,startTick:7680,durationTicks:120,velocity:80},{id:'locked-note',pitch:74,startTick:7800,durationTicks:120,velocity:82,locked:true},{id:'outside-note',pitch:75,startTick:40320,durationTicks:120,velocity:83}]},{id:'other',name:'Other',trackType:'midi-melodic',roleAssignment:'unassigned',notes:[{id:'imported-note',pitch:76,startTick:7680,durationTicks:120,velocity:84}]}]}});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);const core=window.MusicStudioEditor,session=app.state.midiEditor;core.selectTrackById(session,'vocals');const first=app.editorStartPartialEdit();assert.deepEqual(Array.from(first.request.targetNoteIds),[]);app.editorSetEditRange(1,20);const rebased=app.state.partialEditSession;assert.notEqual(rebased,first);assert.equal(rebased.status,'created');assert.equal(rebased.request.trackId,'vocals');assert.equal(rebased.request.range.endMeasure,20);assert.deepEqual(Array.from(rebased.request.targetNoteIds),['imported-note']);assert.deepEqual(Array.from(rebased.request.lockedNoteIds),['locked-note']);core.selectNote(session,'imported-note');assert.equal(app.editorPreviewPartialEditPitchUp().summary.updated,1);assert.equal(app.editorApplyPartialEdit().applied,true);assert.equal(core.getTrackById(session.midiData.tracks,'vocals').notes.find(note=>note.id==='imported-note').pitch,73);assert.equal(core.getTrackById(session.midiData.tracks,'vocals').notes.find(note=>note.id==='locked-note').pitch,74);assert.equal(core.getTrackById(session.midiData.tracks,'vocals').notes.find(note=>note.id==='outside-note').pitch,75);assert.equal(core.getTrackById(session.midiData.tracks,'other').notes.find(note=>note.id==='imported-note').pitch,76);assert.equal(session.undo.length,1);app.editorUndo();assert.equal(core.getTrackById(session.midiData.tracks,'vocals').notes.find(note=>note.id==='imported-note').pitch,72);app.editorRedo();assert.equal(core.getTrackById(session.midiData.tracks,'vocals').notes.find(note=>note.id==='imported-note').pitch,73)
});
...
test('Melody Tempo UI synchronizes initial BPM and applies removes saves and exports tempo changes outside note Undo',async()=>{const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'tempo-change',projectName:'Tempo Change',bpm:120,midiData:{ppq:480,tempo:120,timeSignature:{numerator:4,denominator:4},editor:{measureCount:4},tracks:[{id:'melody',part:'melody',channel:1,program:0,notes:[{id:'n',pitch:60,startTick:0,durationTicks:3840,velocity:90}]}]}});app.setRepository(repo);await repo.put(project);app.state.projects=[project];const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/id="melodyTempoPanel"/);assert.match(html,/Current Tempo/);assert.match(html,/name="tempoChangeMeasure"/);assert.match(html,/name="tempoChangeBpm"[^>]*step="0\.001"/);const session=app.state.midiEditor,undo=session.undo.length;assert.equal(app.editorSetBpm(19).ok,false);assert.equal(app.editorSetBpm(401).ok,false);assert.equal(app.editorSetBpm(137.5).ok,true);assert.equal(session.midiData.tempo,137.5);assert.equal(session.midiData.tempoMap[0].bpm,137.5);assert.equal(app.state.projects[0].musicalSettings.bpm,137.5);session.tempoChangeDraft={measure:2,bpm:90.5};assert.equal(app.editorPreviewTempoChange().ok,true);const applied=app.editorApplyTempoChange();assert.equal(applied.ok,true);assert.equal(JSON.stringify(session.midiData.tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,137.5],[1920,90.5]]));assert.equal(session.undo.length,undo);await app.state.midiEditorSavePromise;const stored=await repo.get(project.projectId);assert.equal(JSON.stringify(stored.midiData.tempoMap.map(item=>[item.tick,item.bpm])),JSON.stringify([[0,137.5],[1920,90.5]]));const made=window.MusicStudioMidi.createMidiFile(app.midiExportInput(stored,'melody'));assert.equal(JSON.stringify(made.inspection.tracks[0].tempoMap.map(item=>item.tick)),JSON.stringify([0,1920]));assert.ok(Math.abs(made.inspection.tracks[0].tempoMap[1].bpm-90.5)<.001);assert.equal(app.editorRemoveTempoChange().ok,true);assert.equal(JSON.stringify(session.midiData.tempoMap.map(item=>item.tick)),JSON.stringify([0]));session.tempoChangeDraft={measure:5,bpm:90};assert.equal(app.editorApplyTempoChange().ok,false);assert.equal(stored.schemaVersion,'1.0');assert.equal(app.APP_VERSION,'1.4.0')});

test('External single Track MIDI export is exact-ID isolated and preserves the SMF contract without mutation',()=>{
  const{app,window}=load(),project=app.makeProject({projectId:'single-external-export',projectName:'AI / Roundtrip',midiData:{version:1,ppq:960,tempo:123.5,tempoMap:[{tick:0,bpm:123.5},{tick:1920,bpm:98}],timeSignature:{numerator:7,denominator:8},totalTick:7680,tracks:[
    {id:'melody',part:'melody',name:'Shared',channel:1,program:0,notes:[{id:'same-note',pitch:60,startTick:0,durationTicks:480,velocity:70}]},
    {id:'drums',part:'drums',name:'Drums',channel:10,program:null,notes:[]},{id:'bass',part:'bass',name:'Bass',channel:2,program:32,notes:[]},
    {id:'external-melody-a',name:'Shared',roleAssignment:'melody',trackType:'midi-melodic',channel:4,program:41,metadata:{source:'keep'},notes:[{id:'same-note',pitch:67,startTick:240,durationTicks:720,velocity:93}]},
    {id:'external-melody-b',name:'Shared',roleAssignment:'melody',trackType:'midi-melodic',channel:5,program:42,notes:[{id:'same-note',pitch:72,startTick:960,durationTicks:360,velocity:82}]},
    {id:'external-drums',name:'AI Drums',roleAssignment:'drums',trackType:'midi-drums',channel:10,program:null,notes:[{id:'same-note',pitch:38,startTick:480,durationTicks:120,velocity:111}]},
    {id:'external-bass',name:'AI Bass',roleAssignment:'bass',trackType:'midi-melodic',channel:6,program:33,notes:[{id:'same-note',pitch:40,startTick:1440,durationTicks:960,velocity:88}]}
  ]}}),before=JSON.stringify(project);
  for(const [trackId,expected] of [['external-melody-b',{name:'Shared',channel:5,program:42,pitch:72,start:960,duration:360,velocity:82}],['external-drums',{name:'AI Drums',channel:10,program:null,pitch:38,start:480,duration:120,velocity:111}],['external-bass',{name:'AI Bass',channel:6,program:33,pitch:40,start:1440,duration:960,velocity:88}]]){
    const summary=app.midiExportSummary(project,'track',trackId),made=window.MusicStudioMidi.createMidiFile(summary.input),parsed=window.MusicStudioMidiParser.parseMidiFile(made.bytes).normalized,track=parsed.tracks.find(item=>item.noteCount);
    assert.equal(summary.validation.ok,true);assert.deepEqual(Array.from(summary.input.tracks,item=>item.id),[trackId]);assert.equal(summary.validation.trackCount,1);assert.equal(parsed.tracks.filter(item=>item.noteCount).length,1);assert.equal(track.name,expected.name);assert.equal(track.channel,expected.channel);assert.equal(track.programChanges[0]?.program??null,expected.program);assert.deepEqual(Array.from(track.notes,note=>[note.pitch,note.startTick,note.durationTicks,note.velocity]),[[expected.pitch,expected.start,expected.duration,expected.velocity]]);assert.equal(parsed.ppq,960);assert.ok(Math.abs(parsed.tempo-123.5)<.001);assert.deepEqual(Array.from(parsed.tempoMap,item=>item.tick),[0,1920]);assert.deepEqual([parsed.timeSignature.numerator,parsed.timeSignature.denominator],[7,8]);assert.equal(JSON.stringify(project),before)
  }
  assert.equal(app.midiExportSummary(project,'track','missing').validation.ok,false);assert.deepEqual(Array.from(app.midiExportTrackInput(project,'external-melody-a').tracks,item=>item.id),['external-melody-a']);assert.equal(JSON.stringify(project),before);assert.equal(project.midiData.tracks.find(track=>track.id==='external-melody-a').metadata.source,'keep');assert.equal(project.midiData.tracks.find(track=>track.id==='external-melody-a').roleAssignment,'melody')
});

test('External current Track download pins selection rejects stale Track and leaves saved Project unchanged',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'single-download',projectName:'Single Download',midiData:{ppq:480,tempo:120,timeSignature:{numerator:4,denominator:4},tracks:[{id:'melody',part:'melody',name:'Melody',channel:1,program:0,notes:[]},{id:'external-a',name:'Twin',roleAssignment:'unassigned',trackType:'midi-melodic',channel:3,program:48,notes:[{id:'same',pitch:65,startTick:0,durationTicks:480,velocity:90}]},{id:'external-b',name:'Twin',roleAssignment:'unassigned',trackType:'midi-melodic',channel:4,program:49,notes:[{id:'same',pitch:69,startTick:480,durationTicks:240,velocity:80}]}]}});app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);window.MusicStudioEditor.selectTrackById(app.state.midiEditor,'external-a');const saved=JSON.stringify(await repo.get(project.projectId)),sessionBefore=JSON.stringify(app.state.midiEditor.midiData),downloads=[];window.URL={createObjectURL:()=>`blob:test`,revokeObjectURL(){}};window.document={body:{dataset:{},appendChild(){}},createElement:()=>({click(){downloads.push(this.download)},remove(){}})};
  const exported=await app.performCurrentTrackMidiExport(project.projectId);assert.equal(exported.ok,true);assert.equal(exported.history.scope,'track');assert.equal(exported.history.targetTrackId,'external-a');assert.equal(exported.validation.trackCount,1);assert.deepEqual(downloads,['Single-Download_Twin.mid']);assert.equal(JSON.stringify(app.state.midiEditor.midiData),sessionBefore);assert.equal(JSON.stringify(await repo.get(project.projectId)),saved);
  window.MusicStudioEditor.selectTrackById(app.state.midiEditor,'external-b');const stale=await app.performMidiExport('track',project.projectId,'external-a');assert.equal(stale.ok,false);assert.equal(stale.reason,'stale-track');assert.equal(downloads.length,1);assert.equal(JSON.stringify(await repo.get(project.projectId)),saved)
});
