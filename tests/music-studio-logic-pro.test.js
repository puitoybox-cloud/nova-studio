...
test('Core and exact-ID External Melody render distinct batch popovers with working Close and reopen contracts',()=>{
  ...
  assert.equal(window.MusicStudioEditor.selectTrackById(app.state.midiEditor,'external-duplicate'),true);html=app.renderRoute(route);assert.equal(app.state.midiEditor.selectedTrackId,'external-duplicate');assert.match(html,/<summary>Transpose \/ Note Length（一括編集）<\/summary>/);assert.match(html,/id="melodyTransposePanel"/);assert.match(html,/id="melodyNoteLengthPanel"/);assert.doesNotMatch(html,/id="melodyCorrectionForm"|editorPreviewCorrection|music-tempo-panel/);assert.equal((html.match(/music-correction-panel-close/g)||[]).length,1);assert.match(html,/music-correction-panel-close" onclick="MusicStudio.editorClosePopup\(this\)"/);
