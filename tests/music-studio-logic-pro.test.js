...
test('Mac Melody editor exposes the temporary Record Audio Diagnostics version and reset controls',()=>{
  const{app}=load({platform:'MacIntel',userAgent:'Mozilla/5.0 (Macintosh; Intel Mac OS X)'}),project=app.makeProject({projectId:'diagnostic-ui',projectName:'Diagnostic UI'});app.state.projects=[project];const html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);
  assert.match(html,/Record Audio Diagnostics/);assert.match(html,/music-studio\.js version/);assert.match(html,/music-studio-audio\.js version/);assert.match(html,/AudioContext存在/);assert.match(html,/recordingScheduledAt/);assert.match(html,/playNotes呼び出し回数/);assert.match(html,/Melody oscillator start\(\)回数/);assert.match(html,/録音中scheduler開始/);assert.match(html,/最後のcleanup名/);assert.match(html,/resetRecordAudioDiagnostics\(\)/);assert.match(fs.readFileSync(path.join(__dirname,'..','music-studio.css'),'utf8'),/Temporary PR #164 hardware audio diagnostics/);
});
...
test('real Record audio path keeps Melody and Metronome nodes through the Count-in boundary until Stop',async()=>{
  ...
  clock=3250;await app.editorStopTransport();assert.equal(synth.playingVoices,0);assert.equal(app.state.melodyAudio.metronomeTimer,null);assert.equal(frame,null);const diagnostic=app.state.recordAudioDiagnostics,audioDiagnostic=synth.diagnostics;assert.equal(diagnostic.recordPressed,true);assert.equal(diagnostic.existingMelodyNotes,1);assert.equal(diagnostic.playbackSchedulerStarted,true);assert.equal(diagnostic.recordMetronomeSchedulerStarted,true);assert.equal(diagnostic.countInCleanupCount,0);assert.equal(diagnostic.recordPlaybackCleanupCount,1);assert.equal(diagnostic.stopCleanupCount,1);assert.equal(diagnostic.lastCleanup,'editorStopTransport');assert.equal(audioDiagnostic.playNotesCalls,1);assert.equal(audioDiagnostic.playbackOscillatorStarts,1);assert.ok(audioDiagnostic.metronomeOscillatorStarts>=5);const diagnosticHtml=app.recordAudioDiagnosticsContent();assert.match(diagnosticHtml,/既存Melodyノート件数/);assert.match(diagnosticHtml,/editorStopTransport/);const noteCount=window.MusicStudioEditor.currentTrack(app.state.midiEditor).notes.length,context=synth.context;app.resetRecordAudioDiagnostics();assert.equal(app.state.recordAudioDiagnostics.recordPressed,false);assert.equal(synth.diagnostics.playNotesCalls,0);assert.equal(synth.context,context);assert.equal(window.MusicStudioEditor.currentTrack(app.state.midiEditor).notes.length,noteCount);assert.equal(app.state.midiInput.recording,false);
});
