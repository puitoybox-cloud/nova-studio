...
test('MIDI Input Assignment is runtime-only and follows the visible Track while idle',async()=>{
  ...
  assert.equal(app.state.midiInput.targetPart,'melody');assert.match(html,/aria-label="Input To"[^>]*>[\s\S]*value="melody" selected[\s\S]*value="drums"[\s\S]*value="bass"/);
  app.state.midiInput.selectedId='device-a';assert.equal(app.editorSelectMidiTarget('drums'),true);assert.equal(app.state.midiInput.targetPart,'drums');assert.equal(app.state.midiInput.selectedId,'device-a');for(const part of ['bass','drums','melody']){app.editorSelectPart(part);assert.equal(app.state.midiEditor.part,part);assert.equal(app.state.midiInput.targetPart,part);html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,new RegExp(`aria-label="Input To"[^>]*>[\\s\\S]*value="${part}" selected`))}assert.equal(app.editorSelectMidiTarget('bass'),true);assert.equal(app.state.midiInput.targetPart,'bass');
  ...
  app.state.midiInput.recording=true;app.state.midiInput.recordingPart='bass';app.state.midiInput.recordingTrackId='bass';assert.equal(app.editorSelectMidiTarget('melody'),false);app.editorSelectPart('drums');assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/aria-label="Input To" disabled/);assert.equal(app.state.midiInput.recordingPart,'bass');assert.equal(app.state.midiInput.recordingTrackId,'bass');app.state.midiInput.recording=false;app.state.midiInput.countingIn=true;assert.equal(app.editorSelectMidiTarget('drums'),false);app.editorSelectPart('melody');assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/aria-label="Input To" disabled/);app.state.midiInput.countingIn=false;
});
