...
test('Remove Measure updates UI bounds Add Note history persistence JSON and MIDI export without changing notes',async()=>{
  ...
  assert.match(html,/editorAddMeasures\(\).*editorRemoveMeasures\(\)/);assert.match(html,/Remove Measure（小節削除）/);session.playheadTick=30000;core.setLoopRange(session,20000,30000,true);
  let removed=app.editorRemoveMeasures();assert.equal(removed.measureCount,12);assert.equal(session.playheadTick,23040);assert.equal(session.midiData.editor.loopStart,20000);assert.equal(session.midiData.editor.loopEnd,23040);assert.match(app.state.notice,/4小節削除/);
  removed=app.editorRemoveMeasures();assert.equal(removed.measureCount,8);app.editorUndo();assert.equal(session.midiData.editor.measureCount,12);await app.state.midiEditorSavePromise;let stored=await repo.get(project.projectId);assert.equal(stored.midiData.editor.measureCount,12);
  app.editorRedo();await app.state.midiEditorSavePromise;stored=await repo.get(project.projectId);assert.equal(stored.midiData.editor.measureCount,8);assert.deepEqual(Array.from(stored.midiData.tracks.find(track=>track.part==='melody').notes,note=>[note.id,note.pitch,note.startTick,note.durationTicks,note.velocity]),[['kept',64,120,360,88]]);
  app.state.projects=[stored];app.state.midiEditor=null;html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);session=app.state.midiEditor;assert.equal(session.midiData.editor.measureCount,8);assert.match(html,/曲の長さ：8小節/);assert.doesNotMatch(html,/aria-label="Bar 9/);
  let json=await app.exportProject(project.projectId);assert.equal(JSON.parse(json.text).midiData.editor.measureCount,8);let backup=app.backupObject();assert.equal(backup.projects.find(item=>item.projectId===project.projectId).midiData.editor.measureCount,8);
  session.playheadTick=14880;app.editorSetSnap('1/16');assert.equal(app.editorAddNote(),true);assert.equal(app.editorAddNote(),true);assert.equal(app.editorAddNote(),true);assert.equal(app.editorAddNote(),true);assert.equal(app.editorAddNote(),false);assert.match(app.state.notice,/小節を追加してください/);app.editorAddMeasures();assert.equal(app.editorAddNote(),true);
  const exported=app.midiExportInput({...stored,midiData:session.midiData},'all');assert.equal(exported.editor.measureCount,12);assert.equal(exported.totalTick,23040);assert.deepEqual(Array.from(exported.tracks.find(track=>track.part==='melody').notes.filter(note=>note.id==='kept'),note=>[note.pitch,note.startTick,note.durationTicks,note.velocity]),[[64,120,360,88]]);
  await app.state.midiEditorSavePromise;json=await app.exportProject(project.projectId);assert.equal(JSON.parse(json.text).midiData.editor.measureCount,12);backup=app.backupObject();assert.equal(backup.projects.find(item=>item.projectId===project.projectId).midiData.editor.measureCount,12);
});
test('Remove Measure blocks notes in Melody Drums or Bass and reports the four-measure minimum',()=>{
  for(const part of ['melody','drums','bass']){const{app,window}=load(),project=app.makeProject({projectId:`remove-${part}`,projectName:`Remove ${part}`,midiData:{editor:{measureCount:8},tracks:[{part:'melody',notes:[]},{part:'drums',channel:10,notes:[]},{part:'bass',notes:[]}]}});app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);window.MusicStudioEditor.selectPart(app.state.midiEditor,part);window.MusicStudioEditor.addNote(app.state.midiEditor,{startTick:7680,durationTicks:120});assert.equal(app.editorRemoveMeasures(),undefined);assert.match(app.state.notice,/削除する小節にノートがあります/);assert.equal(app.state.midiEditor.midiData.editor.measureCount,8)}
...
test('project save restores Zoom, Fit Range, and Piano Roll scroll with legacy defaults',async()=>{
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(app.state.midiEditor.view)),{zoom:1,pitchMin:37,pitchMax:85,pitchScrollTop:null,pitchScrollLeft:0,quantizeEnabled:false,quantize:'1/16'});
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(stored.midiData.editor.view)),{zoom:2,pitchMin:37,pitchMax:85,pitchScrollTop:864,pitchScrollLeft:432,quantizeEnabled:false,quantize:'1/16'});
