...
test('MIDI Input Assignment is runtime-only independent from device and visible Track',async()=>{
  ...
  app.state.midiInput.selectedId='device-a';assert.equal(app.editorSelectMidiTarget('drums'),true);assert.equal(app.state.midiInput.targetPart,'drums');assert.equal(app.state.midiInput.selectedId,'device-a');app.editorSelectPart('bass');assert.equal(app.state.midiInput.targetPart,'drums');assert.equal(app.editorSelectMidiTarget('bass'),true);assert.equal(app.state.midiInput.targetPart,'bass');
  ...
  app.state.midiInput.recording=true;app.state.midiInput.recordingPart='bass';app.state.midiInput.recordingTrackId='bass';assert.equal(app.editorSelectMidiTarget('melody'),false);assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/aria-label="Input To" disabled/);assert.equal(app.state.midiInput.recordingPart,'bass');assert.equal(app.state.midiInput.recordingTrackId,'bass');app.state.midiInput.recording=false;app.state.midiInput.countingIn=true;assert.equal(app.editorSelectMidiTarget('drums'),false);assert.equal(app.state.midiInput.targetPart,'bass');html=app.renderRoute(`music-studio/midi-editor/${project.projectId}`);assert.match(html,/aria-label="Input To" disabled/);app.state.midiInput.countingIn=false;
});
