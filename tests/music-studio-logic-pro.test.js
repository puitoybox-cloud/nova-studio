...
test('one-bar count-in excludes early notes and records the first post-count note at Bar 1',async()=>{
  const{app,window}=load(),repo=app.memoryRepository(),project=app.makeProject({projectId:'count-in-recording',projectName:'Count-in',bpm:120,timeSignature:'4/4'}),input={id:'keys',name:'Keys',onmidimessage:null};app.setRepository(repo);await repo.put(project);app.state.projects=[project];app.renderRoute(`music-studio/midi-editor/${project.projectId}`);
  ...
  const clicks=[],preview=[];app.state.melodyAudio.synth={supported:()=>true,unlock:async()=>true,noteOn(pitch){preview.push(['on',pitch])},noteOff(pitch){preview.push(['off',pitch])},allNotesOff(){},stopPlayback(){},metronomeClick(accent){clicks.push({time:clock,accent})},stopMetronome(){}};Object.assign(app.state.midiInput,{initialized:true,supported:true,access:{},inputs:[input],selectedId:'keys'});await app.editorSelectMidiInput('keys');
  ...
  await advanceTo(3000);const started=await pending;assert.equal(started.recording,true);assert.equal(app.state.midiInput.countingIn,false);assert.equal(app.state.midiInput.recordingStartedAt,3000);assert.deepEqual(clicks.slice(0,5).map(click=>click.time),[1000,1500,2000,2500,3000]);assert.deepEqual(clicks.slice(0,5).map(click=>click.accent),[true,false,false,false,true]);
  input.onmidimessage({data:[0x90,60,100],timeStamp:3000});input.onmidimessage({data:[0x80,60,0],timeStamp:3500});clock=3500;const result=await app.editorStopTransport(),notes=window.MusicStudioEditor.currentTrack(app.state.midiEditor).notes;assert.equal(result.ok,true);assert.equal(notes.length,1);assert.equal(notes[0].pitch,60);assert.equal(notes[0].startTick,0);assert.equal(notes[0].durationTicks,480);assert.equal(frame,null);assert.equal((await repo.get(project.projectId)).midiData.tracks.find(track=>track.part==='melody').notes[0].startTick,0);
});
