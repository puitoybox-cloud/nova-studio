...
test('one-bar count-in excludes early notes and starts existing playback with recording at Bar 1',async()=>{
  ...
  assert.equal(playback.length,1);assert.equal(playback[0].time,1000);assert.equal(playback[0].options.leadSeconds,2);assert.equal(playback[0].notes[0].id,'existing');
