...
test('synthetic local audio response reaches real Track Review, repository reload and Logic Pro MIDI export',async()=>{
  ...
  const editor=window.MusicStudioEditor;
  const session=editor.createSession(stored);
  assert.equal(editor.selectTrackById(session,external[0].id),true);
  const selected=editor.currentTrack(session),noteId=selected.notes[0].id;
  assert.equal(selected.notes[0].velocity,90);
  editor.updateNote(session,noteId,{velocity:97});
  assert.equal(editor.currentTrack(session).notes[0].velocity,97);
  editor.undo(session);
  assert.equal(editor.currentTrack(session).notes[0].velocity,90);
  editor.redo(session);
  assert.equal(editor.currentTrack(session).notes[0].velocity,97);
  await repo.put({...stored,midiData:session.midiData});
  const reloaded=await repo.get(stored.projectId);
  assert.equal(reloaded.midiData.tracks.find(track=>track.id===external[0].id).notes[0].velocity,97);
  assert.equal(reloaded.midiData.tracks.find(track=>track.id===external[1].id).notes[0].velocity,80);
