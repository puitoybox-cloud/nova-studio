...
test('core descriptors resolve their existing audio modes and unknown tracks use a safe pitched fallback',()=>{const playback=load(),descriptors=playback.createPlaybackDescriptors([...tracks(),{id:'external',name:'Strings',channel:3,notes:[]}]);assert.deepEqual(JSON.parse(JSON.stringify(descriptors.map(track=>[track.id,track.audio]))),[
  ['melody',{mode:'melody',fallback:false,reason:null}],['drums',{mode:'drums',fallback:false,reason:null}],['bass',{mode:'bass',fallback:false,reason:null}],['external',{mode:'melody',fallback:true,reason:'missing-core-role'}]
]);assert.deepEqual(JSON.parse(JSON.stringify(playback.resolvePlaybackAudio({id:'unknown',audio:{mode:'drums'}}))),{mode:'melody',fallback:true,reason:'missing-core-role'})});

test('registry-backed descriptors preserve registry order while scheduling remains identity based',()=>{const playback=load(),input=tracks(),registry={tracks:[input[2],input[0],input[1]]},descriptors=playback.createPlaybackDescriptors(registry),active=playback.activePlaybackTracks(registry,{soloByTrackId:{drums:true}});assert.deepEqual(descriptors.map(track=>track.id),['bass','melody','drums']);assert.deepEqual(active.map(track=>track.id),['drums'])});
