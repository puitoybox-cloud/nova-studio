...
test('muted tracks are excluded and solo selects only eligible audible tracks',()=>{const playback=load(),input=tracks(),runtime={mutedByTrackId:{bass:true},soloByTrackId:{drums:true,bass:true}},descriptors=playback.createPlaybackDescriptors(input,runtime),active=playback.activePlaybackTracks(descriptors);assert.deepEqual(active.map(track=>track.id),['drums']);assert.equal(descriptors.find(track=>track.id==='bass').muted,true);assert.equal(input[2].muted,false)});
