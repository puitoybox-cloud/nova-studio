...
test('Capability Registry enables Local Partial Edit for External melodic and drums MIDI without promoting roles',()=>{const{core,selection}=load(),session=core.createSession(project()),model=selection.createSelectionModel(session,core),melodic=model.find(item=>item.id==='ext-a'),drums=core.resolveTrackCapabilities({id:'external-drums',trackType:'midi-drums'}),audio=core.resolveTrackCapabilities({id:'audio',trackType:'audio'});assert.equal(melodic.capabilities.canEditNotes,true);assert.equal(melodic.capabilities.canPlayMidi,true);assert.equal(melodic.capabilities.supportsPitchCorrection,true);assert.equal(melodic.capabilities.supportsPartialEdit,true);assert.equal(melodic.coreSlot,null);assert.equal(drums.supportsPitchCorrection,false);assert.equal(drums.supportsPartialEdit,true);assert.equal(audio.supportsPartialEdit,false)});
...
test('External selection enables Local Partial Edit while Provider remains gated',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio-dynamic-track-selection.js'),'utf8'),app=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8'),blocked=source.match(/const BLOCKED_EXTERNAL_ACTIONS=\[([\s\S]*?)\];/)?.[1]||'',disabled=source.match(/const actionNames=\[([^\]]*)\]/)?.[1]||'';for(const action of ['editorToggleMelodyPlayback','editorRunPartialEditProvider'])assert.ok(blocked.includes(`'${action}'`));for(const action of ['editorStartPartialEdit','editorPreviewPartialEditPitchUp','editorApplyPartialEdit','editorPreviewCorrection','editorApplyCorrection','editorApplyQuantize','editorPreviewTranspose','editorApplyTranspose','editorPreviewNoteLength','editorApplyNoteLength'])assert.doesNotMatch(blocked,new RegExp(`'${action}'`));for(const action of ['editorAddNote','editorCopy','editorPaste','editorDuplicate','editorSelectAllNotes','editorLockSelectedNotes','editorUnlockSelectedNotes','editorMatchDuration','editorMatchVelocity'])assert.match(source,new RegExp(`editActions=\\[[^\\]]*'${action}'`));assert.doesNotMatch(disabled,/editorApplyQuantize/);assert.match(source,/\.music-partial-edit-provider button/);assert.match(source,/External TrackではLocal Previewのみ/);assert.match(source,/originalPartialPreview/);assert.match(source,/externalKeyboardSelection/);assert.match(app,/function externalMelodyCorrectionPopover/);assert.match(app,/supportsPitchCorrection\?externalMelodyCorrectionPopover/);assert.ok(!source.includes("'editorToggleMidiRecording'"));assert.doesNotMatch(source,/BLOCKED_EXTERNAL_ACTIONS=.*editorStartNoteResize/);assert.match(source,/validPinnedGesture/);assert.match(source,/__externalNoteSelectionTrackId/);assert.match(source,/unsupported-external-edit/);assert.match(source,/supportsPartialEdit/)});
...
test('External Import Export control is visible enabled and dispatches the current exact Track ID',()=>{const{core,selection}=load(),source=project(),session=core.createSession(source);core.selectTrackById(session,'ext-b');let summaryArgs,exportArgs;const currentButton={textContent:'Export Melody MIDI（書き出す）',disabled:true,attributes:{onclick:'legacy'},removeAttribute(name){delete this.attributes[name]},setAttribute(name,value){this.attributes[name]=value}},allButton={textContent:'Export All MIDI（全て書き出す）',disabled:false,removeAttribute(){},setAttribute(){}},heading={dataset:{},title:'',textContent:''},page={dataset:{},classList:{add(){},remove(){}},querySelector(selector){if(selector==='#midiEditorTitle')return heading;return null},querySelectorAll(selector){return selector==='.music-editor-transfer button,.music-midi-transfer button'?[currentButton,allButton]:[]}},api={state:{midiEditor:session,projects:[source],midiExporting:false},midiExportSummary(...args){summaryArgs=args;return{validation:{ok:true}}},performMidiExport(...args){exportArgs=args}};assert.equal(selection.enhanceEditor(page,api,core,null),true);assert.deepEqual([summaryArgs[1],summaryArgs[2]],['track','ext-b']);assert.equal(currentButton.textContent,'Export Current Track MIDI（選択Trackを書き出す）');assert.equal(currentButton.disabled,false);assert.equal(currentButton.attributes.onclick,undefined);currentButton.onclick();assert.deepEqual(exportArgs,['track','dynamic-selection','ext-b']);assert.equal(allButton.textContent,'Export All MIDI（全て書き出す）')});
...
function keyEvent(key,extra={}){let prevented=false;return{key,target:{closest:()=>null},preventDefault(){prevented=true},get prevented(){return prevented},...extra}}
function prepareKeyboardSelection(h,noteIds=['a']){h.core.selectTrackById(h.session,'ext-a');for(const [index,id]of noteIds.entries())h.core.selectNote(h.session,id,{additive:index>0});h.api.__externalNoteSelectionTrackId='ext-a';return h.track('ext-a')}

test('External Arrow and Option Arrow reuse Core move resize actions with Snap Grid and one-step Undo Redo',()=>{
  const h=deletionHarness();prepareKeyboardSelection(h);const note=()=>h.track('ext-a').notes.find(item=>item.id==='a');h.session.view.snap='1/8';h.session.view.snapEnabled=true;
  for(const [event,field,expected]of [[keyEvent('ArrowRight'),'startTick',240],[keyEvent('ArrowLeft'),'startTick',0],[keyEvent('ArrowUp'),'pitch',73],[keyEvent('ArrowDown'),'pitch',72],[keyEvent('ArrowUp',{shiftKey:true}),'pitch',84],[keyEvent('ArrowDown',{shiftKey:true}),'pitch',72],[keyEvent('ArrowRight',{altKey:true}),'durationTicks',720],[keyEvent('ArrowLeft',{altKey:true}),'durationTicks',480]]){const undo=h.session.undo.length;assert.equal(h.api.editorHandleShortcut(event),true);assert.equal(note()[field],expected);assert.equal(h.session.undo.length,undo+1);assert.equal(event.prevented,false);h.core.undo(h.session);h.core.redo(h.session)}
});

test('External keyboard editing is exact-ID scoped protects locked notes and preserves boundaries Save Reload',()=>{
  const h=deletionHarness(),track=prepareKeyboardSelection(h,['a','locked-a']),other=plain(h.track('ext-b').notes),coreTracks=plain(h.session.midiData.tracks.slice(0,3));track.notes.find(note=>note.id==='a').pitch=127;track.notes.find(note=>note.id==='a').durationTicks=1;h.session.view.snapEnabled=false;
  assert.equal(h.api.editorHandleShortcut(keyEvent('ArrowUp')),true);assert.equal(track.notes.find(note=>note.id==='a').pitch,127);assert.equal(track.notes.find(note=>note.id==='locked-a').pitch,75);
  assert.equal(h.api.editorHandleShortcut(keyEvent('ArrowLeft',{altKey:true})),true);assert.equal(track.notes.find(note=>note.id==='a').durationTicks,1);assert.equal(track.notes.find(note=>note.id==='locked-a').durationTicks,240);
  assert.equal(h.api.editorHandleShortcut(keyEvent('ArrowDown',{shiftKey:true})),true);assert.equal(track.notes.find(note=>note.id==='a').pitch,115);assert.deepEqual(plain(h.track('ext-b').notes),other);assert.deepEqual(plain(h.session.midiData.tracks.slice(0,3)),coreTracks);
  const saved=JSON.stringify(h.session.midiData),reloaded=h.core.createSession({projectId:'keyboard-reload',midiData:JSON.parse(saved)});assert.equal(JSON.stringify(reloaded.midiData),saved);assert.deepEqual(plain(reloaded.midiData.tracks.find(item=>item.id==='ext-a').notes),plain(track.notes))
});

test('External keyboard shortcuts reject stale Track selection deleted notes typing composition repeat and unsupported modifiers',()=>{
  for(const setup of [h=>{prepareKeyboardSelection(h);h.core.selectTrackById(h.session,'ext-b')},h=>{prepareKeyboardSelection(h);h.session.selectionTrackId='ext-b'},h=>{const track=prepareKeyboardSelection(h);track.notes=track.notes.filter(note=>note.id!=='a')}]){const h=deletionHarness();setup(h);const before=plain(h.session.midiData),undo=h.session.undo.length;assert.equal(h.api.editorHandleShortcut(keyEvent('ArrowRight')),false);assert.deepEqual(plain(h.session.midiData),before);assert.equal(h.session.undo.length,undo)}
  const h=deletionHarness();prepareKeyboardSelection(h);for(const event of [keyEvent('ArrowRight',{isComposing:true}),keyEvent('ArrowRight',{repeat:true}),keyEvent('ArrowRight',{ctrlKey:true}),keyEvent('ArrowRight',{metaKey:true}),keyEvent('ArrowRight',{altKey:true,shiftKey:true}),keyEvent('ArrowRight',{target:{closest:()=>({})}})]){const before=plain(h.session.midiData);assert.equal(h.api.editorHandleShortcut(event),false);assert.deepEqual(plain(h.session.midiData),before);assert.equal(event.prevented,false)}
});

test('Core Melody Drums Bass keyboard shortcuts still delegate unchanged to the original handler',()=>{const h=deletionHarness();for(const part of ['melody','drums','bass']){h.core.selectPart(h.session,part);assert.equal(h.api.editorHandleShortcut(keyEvent('ArrowRight')),true)}});
