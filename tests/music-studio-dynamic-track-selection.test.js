...
test('Capability Registry enables pitch Correction only for External melodic MIDI without promoting roles',()=>{const{core,selection}=load(),session=core.createSession(project()),model=selection.createSelectionModel(session,core),melodic=model.find(item=>item.id==='ext-a'),drums=core.resolveTrackCapabilities({id:'external-drums',trackType:'midi-drums'});assert.equal(melodic.capabilities.canEditNotes,true);assert.equal(melodic.capabilities.canPlayMidi,true);assert.equal(melodic.capabilities.supportsPitchCorrection,true);assert.equal(melodic.capabilities.supportsPartialEdit,false);assert.equal(melodic.coreSlot,null);assert.equal(drums.supportsPitchCorrection,false)});
