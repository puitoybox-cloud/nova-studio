...
test('External selection enables Quantize Transpose scoped note and pointer editing while correction Partial Edit and batch length remain gated',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio-dynamic-track-selection.js'),'utf8'),app=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8'),blocked=source.match(/const BLOCKED_EXTERNAL_ACTIONS=\[([\s\S]*?)\];/)?.[1]||'',disabled=source.match(/const actionNames=\[([^\]]*)\]/)?.[1]||'';for(const action of ['editorToggleMelodyPlayback','editorStartPartialEdit','editorPreviewCorrection','editorPreviewNoteLength','editorApplyNoteLength'])assert.ok(blocked.includes(`'${action}'`));for(const action of ['editorAddNote','editorCopy','editorPaste','editorDuplicate','editorSelectAllNotes','editorLockSelectedNotes','editorUnlockSelectedNotes','editorMatchDuration','editorMatchVelocity'])assert.match(source,new RegExp(`editActions=\\[[^\\]]*'${action}'`));for(const action of ['editorApplyQuantize','editorPreviewTranspose','editorApplyTranspose'])assert.doesNotMatch(blocked,new RegExp(action));assert.doesNotMatch(disabled,/editorApplyQuantize/);assert.match(source,/\.music-transpose-panel button/);assert.match(app,/correction=melody\|\|externalMidi\?melodyCorrectionPopover/);assert.ok(!source.includes("'editorToggleMidiRecording'"));assert.doesNotMatch(source,/BLOCKED_EXTERNAL_ACTIONS=.*editorStartNoteResize/);assert.match(source,/validPinnedGesture/);assert.match(source,/__externalPointerGesture/);assert.match(source,/unsupported-external-edit/);assert.match(source,/supportsPartialEdit/)});
...
test('standalone HTML loads Dynamic Selection CSS and JS after the existing editor stack',()=>{const html=fs.readFileSync(path.join(__dirname,'..','music-studio.html'),'utf8');assert.match(html,/music-studio-dynamic-track-selection\.css\?v=1\.0\.0/);assert.match(html,/music-studio-dynamic-track-selection\.js\?v=1\.0\.7/);assert.ok(html.indexOf('music-studio.js?v=1.4.98')<html.indexOf('music-studio-dynamic-track-selection.js?v=1.0.7'))});
test('Nova Studio host loads Dynamic Selection assets without changing Music Studio APP_VERSION or schemaVersion',()=>{const html=fs.readFileSync(path.join(__dirname,'..','index.html'),'utf8'),app=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(html,/music-studio-dynamic-track-selection\.css\?v=1\.0\.0/);assert.match(html,/music-studio-dynamic-track-selection\.js\?v=1\.0\.7/);assert.match(app,/const APP_VERSION='1\.4\.0'/);assert.match(app,/const SCHEMA_VERSION='1\.0'/)});
