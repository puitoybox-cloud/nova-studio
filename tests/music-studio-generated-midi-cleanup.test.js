...
test('cleanup reuses the Import Export placement and Melody Correction controls',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8'),css=fs.readFileSync(path.join(__dirname,'..','music-studio.css'),'utf8');assert.match(source,/music-editor-popover music-editor-transfer music-cleanup-popover/);for(const name of ['music-correction-heading','music-note-length-value','music-correction-group','music-correction-target','music-correction-actions','music-help'])assert.match(source,new RegExp(`class="${name}`));assert.doesNotMatch(css,/\.music-cleanup-popover\s*\{[^}]*(?:position|top|right|bottom|left|max-height)\s*:/);assert.match(css,/:is\(\.music-correction-popover,\.music-cleanup-popover\)/);assert.match(css,/\.nova-unified-content:has\(>\.music-midi-editor-page\),\.nova-studio-route-content:has\(>\.music-midi-editor-page\)\{overflow:visible;-webkit-backdrop-filter:none;backdrop-filter:none\}/)});

test('cleanup menu open state survives editor repaint',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/generatedCleanupMenuOpen=cleanupMenu\.open/);assert.match(source,/preview\|\|session\.view\?\.generatedCleanupMenuOpen\?'open':''/);assert.match(source,/if\(cleanup\)view\.generatedCleanupMenuOpen=active\.open/)});

test('cleanup Cancel discards only Preview while Close remains the panel action',()=>{const core=load(),session=core.createSession(project()),before=plain(session.midiData);core.previewGeneratedMidiCleanup(session);const result=core.cancelGeneratedMidiCleanup(session);assert.deepEqual(plain(result),{ok:true,cancelled:true});assert.equal(session.generatedCleanupPreview,null);assert.deepEqual(plain(session.midiData),before);const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/Cancel Preview（確認を取り消す）/);assert.doesNotMatch(source,/music-cleanup-panel-close/)});

test('the common popup Close injector includes Cleanup and excludes only Correction',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/\(\?!\[\^\\"\]\*music-correction-popover\)/);assert.doesNotMatch(source,/music-correction-popover\|music-cleanup-popover/);assert.equal((source.match(/class="music-secondary music-popup-close"/g)||[]).length,1)});

test('Cleanup and Correction panels persist independent native open state',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/if\(correction\)view\.correctionMenuOpen=active\.open/);assert.match(source,/if\(cleanup\)view\.generatedCleanupMenuOpen=active\.open/);assert.match(source,/generatedCleanupMenuOpen=cleanupMenu\.open/)});
test('every Editor popover has an immediate delegated native toggle handler that closes open siblings before repaint setup',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/addEventListener\?\.\('toggle',[^;]+music-editor-topbar > \.music-editor-menu[^;]+,true\)/);assert.match(source,/querySelectorAll\?\.\('\.music-editor-topbar > \.music-editor-menu'\)/);assert.match(source,/if\(other!==active&&other\.open\)other\.open=false/);assert.match(source,/api\.editorHandlePopupToggle=editorHandlePopupToggle/)});
