...
test('Cleanup and Correction fixed panels are mutually exclusive',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/if\(other===menu\)view\.correctionMenuOpen=false/);assert.match(source,/if\(other===cleanupMenu\)view\.generatedCleanupMenuOpen=false/);assert.match(source,/menu\.ontoggle=\(\)=>\{view\.correctionMenuOpen=menu\.open;if\(menu\.open\)\{closeOtherEditorMenus\(menu\)/);assert.match(source,/cleanupMenu\.ontoggle=\(\)=>\{view\.generatedCleanupMenuOpen=cleanupMenu\.open;if\(cleanupMenu\.open\)closeOtherEditorMenus\(cleanupMenu\)/)});
test('every Editor popover closes its open siblings instead of stacking Close controls',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/querySelectorAll\?\.\('\.music-editor-topbar > \.music-editor-menu'\)/);assert.match(source,/const closeOtherEditorMenus=active=>\{for\(const other of editorMenus\)/);assert.match(source,/if\(editorMenu\.open\)closeOtherEditorMenus\(editorMenu\)/);assert.match(source,/if\(menu\.open\)\{closeOtherEditorMenus\(menu\)/);assert.match(source,/if\(cleanupMenu\.open\)closeOtherEditorMenus\(cleanupMenu\)/)});
