...
test('cleanup fixed popover has an explicit viewport top and its host ancestor cannot establish a Chrome fixed containing block',()=>{const css=fs.readFileSync(path.join(__dirname,'..','music-studio.css'),'utf8');const rule=css.match(/\.music-cleanup-popover\{([^}]+)\}/)?.[1]||'';assert.match(rule,/position:fixed/);assert.match(rule,/top:max\(76px,env\(safe-area-inset-top\)\)/);assert.doesNotMatch(rule,/top:calc\(100%/);assert.match(css,/\.nova-unified-content:has\(>\.music-midi-editor-page\),\.nova-studio-route-content:has\(>\.music-midi-editor-page\)\{overflow:visible;-webkit-backdrop-filter:none;backdrop-filter:none\}/)});
...
test('the common popup Close injector excludes Cleanup because Cleanup owns its dedicated Close',()=>{const source=fs.readFileSync(path.join(__dirname,'..','music-studio.js'),'utf8');assert.match(source,/music-correction-popover\|music-cleanup-popover/);assert.equal((source.match(/class="music-secondary music-cleanup-panel-close"/g)||[]).length,1)});
