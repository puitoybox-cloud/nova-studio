...
final class MusicStudioAppConfigurationTests: XCTestCase {
    ...
    func testVerificationMarkerIsExplicitAndDoesNotChangeFileURLPolicy() {
        let start = URL(fileURLWithPath: "/tmp/music-studio.html")
        let configuration = MusicStudioAppConfiguration(
            startURL: start,
            verificationMarker: "PR #250 HEAD test-revision"
        )

        XCTAssertEqual(configuration.verificationMarker, "PR #250 HEAD test-revision")
        XCTAssertTrue(configuration.allows(start))
    }
}
