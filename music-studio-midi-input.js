...
(function(root){
  ...
  function createRecorder(options={}){
    ...
    let recording=false,startedAt=0,sequence=0;const active=new Map(),notes=[];
    const toTick=milliseconds=>Math.max(0,Math.round(milliseconds*ppq*tempo/60000));
    function start(time=clock()){recording=true;startedAt=Number(time);sequence=0;active.clear();notes.length=0;return api}
  ...
  root.MusicStudioMidiInput={isSupported,createRecorder,requestAccess};
