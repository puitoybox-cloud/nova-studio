...
LOG_FILE="/tmp/nova-studio-pr239-preview.log"

if ! command -v python3 >/dev/null 2>&1; then
  osascript -e 'display alert "PR #239 Previewを起動できません" message "python3が見つかりません。画面を閉じずにティアへ知らせてください。" as critical' 2>/dev/null || true
...
for _ in 1 2 3 4 5; do
  if curl --silent --fail "http://127.0.0.1:${PREVIEW_PORT}/music-studio.html" >/dev/null; then
    ...
    echo "PR #239 Previewを起動しました。"
    echo "対象commit: $PREVIEW_SHA"
    echo "確認URL: $PREVIEW_URL"
    echo "この画面は閉じて構いません。Preview停止時はSTOP_PR239_PREVIEW.commandをダブルクリックしてください。"
    exit 0
  fi
  sleep 1
done
