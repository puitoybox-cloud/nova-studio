...
cd "$(dirname "$0")"
PORT=8765
PRODUCT_SHA="8098b5e2cf1f2c65d6c6dec6f1298fd21d83804f"
echo "PR #250 exact product HEAD: $PRODUCT_SHA"
if [ -f /tmp/pr250-scroll-fix-preview.pid ]; then
  OLD_PID=$(cat /tmp/pr250-scroll-fix-preview.pid 2>/dev/null || true)
  [ -z "$OLD_PID" ] || kill "$OLD_PID" 2>/dev/null || true
  rm -f /tmp/pr250-scroll-fix-preview.pid
fi
python3 -m http.server "$PORT" --bind 0.0.0.0 >/tmp/pr250-scroll-fix-preview.log 2>&1 &
echo $! >/tmp/pr250-scroll-fix-preview.pid
sleep 1
IP=""
