...
(function(){
  ...
  const DREAM_STATUS={existing:'既存アプリあり',available:'利用可能',priority:'優先制作',preparing:'連携準備中',planned:'未実装',review:'確認待ち'};
  const DREAM_GROUPS=[
    {id:'music',label:'音楽・音声',description:'音楽、MIDI、歌詞と音声の制作。Logic Pro周辺を次の優先領域にします。'},
    {id:'art',label:'イラスト・漫画',description:'画像、漫画、LINEスタンプの制作入口です。'},
    {id:'video',label:'映像・アニメ',description:'AIアニメ、Viduプロンプト、動画制作の入口です。'},
    {id:'publish',label:'公開・管理',description:'公開準備、プロンプト管理、今後の制作アプリをまとめます。'}
  ];
  const DREAM_APPS=[
    {id:'music-production',group:'music',icon:'🎵',title:'音楽制作',description:'楽曲の構想、Logic Pro作業、制作メモへ進む入口です。',status:'priority',priority:true,integration:'Nova Studio連携あり',action:"openApp('musicStudio')",features:['楽曲構成と制作メモ','Logic Pro作業の整理','MIDI・歌詞制作への移動']},
    ...
    {id:'voice',group:'music',icon:'🎙️',title:'ボイス制作',description:'台詞、ナレーション、音声素材を制作する予定です。',status:'planned',integration:'Nova Studio連携予定',route:'dream-voice',features:['台詞リスト','声の方向性管理','音声素材候補の確認']},
    ...
    {id:'ai-anime',group:'video',icon:'🎬',title:'AIアニメ制作',description:'既存のProduction Dashboardで作品と話数の制作工程を確認します。',status:'existing',integration:'Nova Studio連携あり',action:'openProductionDashboard()',features:['制作工程管理','Scene進捗確認','完成状況の確認']},
    {id:'vidu-prompt',group:'video',icon:'✨',title:'Viduプロンプト作成',description:'既存のPrompt StudioでVidu向けプロンプトを構成します。',status:'existing',integration:'Nova Studio連携あり',action:"openApp('promptStudio')",features:['日本語・英語プロンプト','キャラクター設定参照','テンプレート管理']},
    {id:'video',group:'video',icon:'🎞️',title:'動画制作',description:'編集工程と動画素材を扱うワークスペースを準備中です。',status:'preparing',integration:'Nova Studio連携準備中',route:'dream-video',features:['素材整理','編集工程管理','完成動画候補の確認']},
    ...
    {id:'prompt-management',group:'publish',icon:'🗂️',title:'プロンプト管理',description:'用途別プロンプトを横断管理するワークスペースを準備中です。',status:'preparing',integration:'Nova Studio連携準備中',route:'dream-prompt-management',features:['プロンプト一覧','用途別分類','作品・話数との紐付け']},
    {id:'future',group:'publish',icon:'＋',title:'今後追加する制作アプリ',description:'新しい制作アプリを安全に追加するための予約入口です。',status:'review',integration:'接続仕様確認待ち',route:'dream-future-apps',features:['追加候補の整理','連携範囲の確認','安全な起動方式の決定']}
  ];
  ...
  function dreamContext(){
    ...
    return {projectName:String(project.name||transfer?.projectName||activeProject?.title||''),projectId:String(project.id||transfer?.projectId||activeProject?.id||''),episodeName:String(episode.name||activeEpisode?.numberLabel||''),episodeId:String(episode.id||transfer?.episodeId||activeEpisode?.id||''),characters:Array.isArray(transfer?.characters)?transfer.characters.length:0,assets:Array.isArray(transfer?.assets)?transfer.assets.length:0,results:resultItems.length,invalid:v2.invalid||v1.invalid||results.invalid,shared:Boolean(transfer)};
  }
  function dreamStatusBadge(status){return `<span class="dream-status is-${status}">${esc(DREAM_STATUS[status]||status)}</span>`}
  function dreamAppCard(item){const action=item.action||`setView('${item.route}')`;const live=Boolean(item.action);return `<article class="dream-app-card is-${item.status}${item.priority?' is-priority':''}"><div class="nova-section-card-top"><span class="nova-section-icon" aria-hidden="true">${item.icon}</span>${dreamStatusBadge(item.status)}</div><h3>${esc(item.title)}</h3><p>${esc(item.description)}</p><dl><div><dt>状態</dt><dd>${esc(DREAM_STATUS[item.status])}</dd></div><div><dt>連携</dt><dd>${esc(item.integration)}</dd></div></dl><button class="${live?'primary-action':'secondary'}" onclick="${action}">${live?'開く':'準備中画面を開く'}</button></article>`}
  function dreamContextPanel(ctx){const selected=ctx.projectId||ctx.projectName;return `<section class="dream-context home-panel" aria-labelledby="dreamContextTitle"><div class="section-head"><div><p class="eyebrow">Nova Studio Link</p><h2 id="dreamContextTitle">現在の制作対象</h2></div><button class="secondary" onclick="setView('dream-architect-link')">共有内容を確認</button></div>${selected?`<dl class="dream-context-grid"><div><dt>作品名</dt><dd>${esc(ctx.projectName||'名称未設定')}</dd></div><div><dt>作品ID</dt><dd>${esc(ctx.projectId||'未設定')}</dd></div><div><dt>選択中の話数</dt><dd>${esc(ctx.episodeName||ctx.episodeId||'未選択')}</dd></div><div><dt>共有キャラクター</dt><dd>${ctx.characters}件</dd></div><div><dt>共有素材</dt><dd>${ctx.assets}件</dd></div><div><dt>受け取り候補</dt><dd>${ctx.results}件</dd></div></dl>`:'<p class="dream-empty-note">Nova Studioから作品が選択されていません。Nova Studioで作品を選び、必要に応じて共有内容を準備してください。</p>'}${ctx.invalid?'<p class="dream-warning" role="status">一部の共有データを読み取れませんでした。既存データは変更せず、安全な値で表示しています。</p>':''}</section>`}
  function dreamArchitectView(){const ctx=dreamContext();const externalConfigured=(state?.apps||[]).some(app=>['productionDashboard','promptStudio','musicStudio'].includes(app.id)&&app.url);return `<main class="dream-architect-home" aria-label="Dream Architect Studio"><header class="dream-architect-hero"><div><p class="eyebrow">Creation App Hub</p><h1>Dream Architect Studio</h1><p>制作アプリをまとめる、Nova Studioとは別の制作室です。</p></div><button class="secondary" onclick="novaReturnHome()">← Nova Studioへ戻る</button></header>${dreamContextPanel(ctx)}<section class="dream-priority home-panel"><p class="eyebrow">Next Priority</p><h2>次に優先する制作項目</h2><p>全体の入口を整えた後は、音楽制作・MIDI Composer・歌詞／音符割付とLogic Pro周辺を優先します。今回は大規模な制作機能追加を行いません。</p></section>${externalConfigured?'':`<aside class="dream-connection-note" role="status"><b>外部アプリは未接続です</b><span>登録URLを推測して開くことはありません。既存の内部画面は利用でき、外部接続先はNova Studioの設定から登録できます。</span><button class="secondary" onclick="setView('settings')">接続設定を確認</button></aside>`}<section class="home-panel dream-apps-panel"><div class="section-head"><div><p class="eyebrow">Creative Apps</p><h2>制作アプリ一覧</h2><p class="meta">状態とNova Studio連携を文字でも表示します。</p></div></div><div class="dream-groups">${DREAM_GROUPS.map(group=>`<section class="dream-group ${group.id==='music'?'is-music':''}" aria-labelledby="dream-group-${group.id}"><div class="dream-group-heading"><div><p class="eyebrow">${String(DREAM_APPS.filter(app=>app.group===group.id).length).padStart(2,'0')} Apps</p><h2 id="dream-group-${group.id}">${esc(group.label)}</h2></div><p>${esc(group.description)}</p></div><div class="dream-app-grid">${DREAM_APPS.filter(app=>app.group===group.id).map(dreamAppCard).join('')}</div></section>`).join('')}</div></section></main>`}
