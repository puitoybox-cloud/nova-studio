...
(function(root){
  ...
  function activePlaybackTracks(tracks=[],playbackState={}){const descriptors=createPlaybackDescriptors(tracks,playbackState),soloed=descriptors.filter(track=>track.solo&&track.soloEligible&&!track.muted);return(soloed.length?soloed:descriptors.filter(track=>!track.muted)).map(clone)}
