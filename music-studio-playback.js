...
(function(root){
  ...
  function activePlaybackTracks(tracks=[]){const descriptors=createPlaybackDescriptors(tracks),soloed=descriptors.filter(track=>track.solo&&track.soloEligible&&!track.muted);return(soloed.length?soloed:descriptors.filter(track=>!track.muted)).map(clone)}
  function createTransportState(midiData={},runtime={}){
    const editor=midiData.editor||{},loopStart=Number(editor.loopStart),loopEnd=Number(editor.loopEnd),hasLoop=editor.loopEnabled===true&&Number.isFinite(loopStart)&&Number.isFinite(loopEnd)&&loopStart>=0&&loopEnd>loopStart,tracks=createPlaybackDescriptors(midiData.tracks||[],runtime.playbackState||runtime);
    return{playing:runtime.playing===true,currentTick:Math.max(0,Number(runtime.currentTick)||0),ppq:Number(midiData.ppq)||480,bpm:Number(midiData.tempo)||120,loopEnabled:hasLoop,loopStart:hasLoop?loopStart:null,loopEnd:hasLoop?loopEnd:null,playbackStartOrigin:Number.isFinite(Number(runtime.playbackStartOrigin))?Number(runtime.playbackStartOrigin):null,activePlaybackTrackIds:activePlaybackTracks(tracks).map(track=>track.id)}
  }
  function schedulePlaybackTracks(synth,tracks=[],transportState={}){
    const active=activePlaybackTracks(tracks),timing={ppq:transportState.ppq,tempo:transportState.bpm??transportState.tempo,startTick:transportState.startTick??transportState.currentTick,endTick:transportState.endTick,startTime:transportState.startTime,leadSeconds:transportState.leadSeconds};
