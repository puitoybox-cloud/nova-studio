  ...
  function inspectMidiBytes(source){
    ...
    try{
      ...
      let position=8+headerLength;for(let index=0;index<trackCount;index++){
        ...
        const length=readU32(bytes,position+4),start=position+8,end=start+length;if(end>bytes.length)throw Error(`トラック${index+1}の長さがファイル境界を超えます。`);let cursor=start,runningStatus=null,endOfTrack=false,eventCount=0,tempo=null,timeSignature=null,name='',tick=0,endOfTrackTick=0,tempoMap=[];
        ...
        if(!endOfTrack)errors.push(`トラック${index+1}にEnd of Trackがありません。`);tracks.push({index,length,eventCount,endOfTrack,endOfTrackTick,name,tempo,tempoMap,timeSignature});position=end;
      }
