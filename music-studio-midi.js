...
(function(root){
  ...
  function validateMidiProjectData(input){
    ...
    return{ok:errors.length===0,errors,warnings,data:{version:Number(data.version)||1,ppq:ppq??480,tempo:Number.isFinite(tempo)?tempo:120,timeSignature:timeSignature||{numerator:4,denominator:4},totalTick:totalTick??noteEnd,tracks},trackCount:tracks.filter(track=>track.notes.length).length,noteCount};
  }
  ...
  function inspectMidiBytes(source){
    ...
    try{
      ...
      let position=8+headerLength;for(let index=0;index<trackCount;index++){
        ...
        const length=readU32(bytes,position+4),start=position+8,end=start+length;if(end>bytes.length)throw Error(`トラック${index+1}の長さがファイル境界を超えます。`);let cursor=start,runningStatus=null,endOfTrack=false,eventCount=0,tempo=null,timeSignature=null,name='',tick=0,endOfTrackTick=0;
        while(cursor<end){const delta=readVlq(bytes,cursor,end);cursor=delta.position;tick+=delta.value;if(cursor>=end)throw Error(`トラック${index+1}のイベントが途中で終わっています。`);let status=bytes[cursor++];if(status<0x80){if(runningStatus===null)throw Error('Running Statusの開始状態がありません。');cursor--;status=runningStatus}else if(status<0xf0)runningStatus=status;else runningStatus=null;
          if(status===0xff){if(cursor>=end)throw Error('Meta Eventが破損しています。');const kind=bytes[cursor++],size=readVlq(bytes,cursor,end);cursor=size.position;if(cursor+size.value>end)throw Error('Meta Eventの長さが不正です。');const payload=bytes.slice(cursor,cursor+size.value);cursor+=size.value;if(kind===0x2f){if(size.value!==0)errors.push('End of Trackの長さが0ではありません。');endOfTrack=true;endOfTrackTick=tick}if(kind===0x51&&size.value===3)tempo=Math.round(60000000/((payload[0]<<16)|(payload[1]<<8)|payload[2])*1000)/1000;if(kind===0x58&&size.value===4)timeSignature=`${payload[0]}/${2**payload[1]}`;if(kind===0x03){try{name=typeof root.TextDecoder==='function'?new root.TextDecoder().decode(payload):String.fromCharCode(...payload)}catch(_){name=''}}
        ...
        if(!endOfTrack)errors.push(`トラック${index+1}にEnd of Trackがありません。`);tracks.push({index,length,eventCount,endOfTrack,endOfTrackTick,name,tempo,timeSignature});position=end;
      }
      if(position!==bytes.length)errors.push('最終トラック後に余分なバイトがあります。');if(trackCount!==tracks.length)errors.push('ヘッダーのトラック数と実チャンク数が一致しません。');if(noteOnCount!==noteOffCount)errors.push('Note OnとNote Offの数が一致しません。');return{ok:errors.length===0,errors,warnings,size:bytes.length,headerLength,type,trackCount,ppq,totalTick:Math.max(0,...tracks.map(track=>track.endOfTrackTick)),tracks,noteOnCount,noteOffCount};
