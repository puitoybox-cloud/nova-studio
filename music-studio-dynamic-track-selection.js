...
(function(root){
  ...
  function install(api=root.MusicStudio,core=root.MusicStudioEditor,doc=root.document){
    ...
    const originalDrag=api.editorStartNoteDrag;if(typeof originalDrag==='function')api.editorStartNoteDrag=function(event,id,...args){if(!isExternalSelection(api,core))return originalDrag.call(this,event,id,...args);if(event?.button!=null&&event.button!==0)return false;event?.preventDefault?.();const additive=Boolean(event?.shiftKey||event?.metaKey||event?.ctrlKey||api.state?.midiMultiSelect),current=resolveCurrentSelection(api.state?.midiEditor,core);api.editorSelectNote?.(id,additive);api.__externalNoteSelectionTrackId=current?.id||null;return true};
