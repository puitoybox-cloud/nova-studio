...
(function(root){
  ...
  function syncExternalNoteSelection(doc,session,core){const selected=new Set(core?.selectedIds?.(session)||[]),page=doc?.querySelector?.('.music-midi-editor-page');page?.querySelectorAll?.('.music-midi-note').forEach(note=>{const active=selected.has(note.dataset?.noteId);note.classList?.toggle?.('is-selected',active);note.setAttribute?.('aria-pressed',String(active))});const canDelete=core?.editableNotes?.(core?.selectedNotes?.(session)||[]).length>0;page?.querySelectorAll?.('[onclick*="editorDeleteNote"]').forEach(control=>{control.disabled=!canDelete;if(canDelete)control.removeAttribute?.('aria-disabled');else control.setAttribute?.('aria-disabled','true')});return selected}
  ...
  function install(api=root.MusicStudio,core=root.MusicStudioEditor,doc=root.document){
    ...
    const originalDrag=api.editorStartNoteDrag;if(typeof originalDrag==='function')api.editorStartNoteDrag=function(event,id,...args){if(!isExternalSelection(api,core))return originalDrag.call(this,event,id,...args);if(event?.button!=null&&event.button!==0)return false;event?.preventDefault?.();const session=api.state?.midiEditor,additive=Boolean(event?.shiftKey||event?.metaKey||event?.ctrlKey||api.state?.midiMultiSelect),current=resolveCurrentSelection(session,core),trackId=current?.id||null;core.selectNote?.(session,id,{toggle:additive});syncExternalNoteSelection(doc,session,core);return captureExternalNoteSelection(api,core,trackId)};
