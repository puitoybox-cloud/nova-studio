...
async function selectTrack(page,trackId){
  const selected=await page.evaluate(id=>MusicStudio.editorSelectTrack(id),trackId);
  assert.equal(selected,true,`${trackId}: selection API must accept Track ID`);
...
async function recordTrack(page,trackId,pitch){
  ...
  const disabled=await page.evaluate(()=>document.querySelector('[onclick*="editorToggleMidiRecording"]')?.disabled);
  assert.equal(disabled,false,`${trackId}: record must be enabled`);
  await page.evaluate(()=>MusicStudio.editorToggleMidiRecording());
  ...
  await page.evaluate(()=>MusicStudio.editorStopTransport());
...
async function verifyKeyboardShortcuts(page){
  for(const trackId of ['melody','drums','bass']){
    ...
    assert.equal(await page.evaluate(()=>MusicStudio.state.midiInput.recordingTrackId),trackId,`${trackId}: R must start exact Core Track recording`);
    ...
    assert.equal(await page.evaluate(()=>MusicStudio.state.midiEditor.playheadTick),0,`${trackId}: Enter must Go to Start`)
  }
  ...
  assert.equal(await page.evaluate(()=>MusicStudio.state.midiEditor.playheadTick),0,'External Enter must Go to Start');
...
async function verifyCountInRouting(page){
  await selectTrack(page,'external-b');
  await page.evaluate(()=>{MusicStudio.state.midiEditor.midiData.editor.transport.countInEnabled=true});
  await page.keyboard.press('r');
  await page.waitForFunction(()=>MusicStudio.state.midiInput.countingIn===true);
  assert.equal(await page.evaluate(()=>MusicStudio.state.midiInput.recordingTrackId),'external-b','Count-in must retain exact selected External Track ID');
  await page.evaluate(()=>MusicStudio.editorStopTransport());
  await page.waitForFunction(()=>MusicStudio.state.midiInput.countingIn===false&&MusicStudio.state.midiInput.recording===false);
  await page.evaluate(()=>{MusicStudio.state.midiEditor.midiData.editor.transport.countInEnabled=false})
}
async function verify(browser,viewport){
  ...
  await verifyCountInRouting(page);
  await recordTrack(page,'external-b',67);
  await recordTrack(page,'external-drums',38);
  await selectTrack(page,'external-b');
  await page.evaluate(()=>MusicStudio.editorToggleMelodyPlayback());await page.waitForTimeout(30);
  assert.deepEqual(await page.evaluate(()=>window.__playedTrackIds||[]),['external-b'],`${viewport.width}: playback must stay on exact selected Track ID`);
  ...
  await page.close();return{viewport:viewport.width,exactTrackId:true,externalMelodicRecord:true,externalDrumRecord:true,coreMelodyShortcut:true,coreDrumsShortcut:true,coreBassShortcut:true,externalRecordShortcut:true,countInExactId:true,goToStartShortcut:true,playbackShortcut:true,liveNote:true,stopCommit:true,playback:true,correctionGate:true,partialEditGate:true,horizontalOverflow:false,consoleErrors:0,consoleWarnings:0}
}
