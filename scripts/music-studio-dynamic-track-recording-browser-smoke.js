...
async function verify(browser,viewport){
  ...
  await page.locator('.music-dynamic-track-group[data-track-id="external-b"] .music-dynamic-track-button').click();
  await page.waitForFunction(()=>MusicStudio.resolveCurrentTrackSelection().id==='external-b');
  ...
  await page.close();return{viewport:viewport.width,exactTrackId:true,externalMelodicRecord:true,externalDrumRecord:true,liveNote:true,stopCommit:true,playback:true,correctionGate:true,partialEditGate:true,horizontalOverflow:false,consoleErrors:0,consoleWarnings:0}
}
