...
if(!environments[provider]||process.argv.length>4||diagnosticMode!==undefined&&(provider!=='gemini'||!['no-schema','no-response-format','no-generation-config'].includes(diagnosticMode))){
  process.stderr.write('Usage: node scripts/music-studio-live-api-smoke.js <openai|gemini> [no-schema|no-response-format|no-generation-config]\n');
  process.exitCode=2;
}else{
