...
async function verifyBatchMenu(page){
  const menu=page.locator('.music-correction-menu');
  assert.equal(await menu.count(),1);assert.equal(await menu.isVisible(),true);assert.equal(await menu.locator('summary').innerText(),'Melody Correction（メロディ補正）');
  await selectTrack(page,'external-melody');assert.equal(await page.locator('.music-midi-editor-page').getAttribute('data-current-track-id'),'external-melody');
  assert.equal(await menu.count(),1);assert.equal(await menu.isVisible(),true);assert.equal(await menu.locator('summary').innerText(),'Transpose / Note Length（一括編集）');
  await menu.locator('summary').click();assert.equal(await menu.getAttribute('open'),'');assert.equal(await menu.locator('.music-transpose-panel').isVisible(),true);assert.equal(await menu.locator('.music-note-length-panel').isVisible(),true);
  assert.equal(await menu.locator('[onclick="MusicStudio.editorPreviewTranspose()"]') .isEnabled(),true);assert.equal(await menu.locator('[onclick="MusicStudio.editorPreviewNoteLength()"]') .isEnabled(),true);assert.equal(await menu.locator('[onclick="MusicStudio.editorPreviewCorrection()"]') .isEnabled(),false);
  await selectTrack(page,'melody');assert.equal(await page.locator('.music-correction-menu summary').innerText(),'Melody Correction（メロディ補正）');
  await selectTrack(page,'external-melody');assert.equal(await page.locator('.music-midi-editor-page').getAttribute('data-current-track-id'),'external-melody');assert.equal(await page.locator('.music-correction-menu summary').innerText(),'Transpose / Note Length（一括編集）');assert.equal(await page.locator('.music-correction-menu').isVisible(),true)
}
...
async function editTrack(page,trackId){
  ...
  const deleteId=await page.evaluate(()=>MusicStudioEditor.currentTrack(MusicStudio.state.midiEditor).notes.find(note=>!note.locked).id);await page.locator(`.music-midi-note[data-note-id="${deleteId}"]`).dispatchEvent('pointerdown',{button:0,pointerId:13,pointerType:'mouse'});const countBeforeDelete=(await snapshot(page))[trackId].length;await page.keyboard.press('Delete');assert.equal((await snapshot(page))[trackId].length,countBeforeDelete-1);await assertOtherTracksUnchanged(page,trackId,before)
}
...
async function verify(browser,viewport){
  ...
  await page.waitForSelector('.music-midi-editor-page');await verifyBatchMenu(page);const coreBefore=await page.evaluate(ids=>Object.fromEntries(MusicStudio.state.midiEditor.midiData.tracks.filter(track=>ids.includes(track.id)).map(track=>[track.id,JSON.parse(JSON.stringify(track.notes))])),coreIds);
