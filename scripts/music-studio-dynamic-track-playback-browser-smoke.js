...
async function verifyViewport(browser,viewport){
  ...
  assert.equal(state.current.id,'external-target');assert.equal(state.selectedTrackId,'external-target');assert.equal(state.pianoNotes.length,12);assert.deepEqual(state.scheduled[0].map(track=>track.id),['external-target']);assert.equal(state.scheduled[0][0].noteIds.length,12);assert.deepEqual(state.activeIds,['external-target']);assert.equal(state.mode,'playback');assert.equal(state.recordDisabled,false,`${viewport.width}: External recording must remain available`);assert.equal(state.batchEnabled,true,`${viewport.width}: External Transpose / Note Length must remain available`);assert.equal(state.partialEnabled,false,`${viewport.width}: partial state ${JSON.stringify(state)}`);assert.ok(state.overflow<=1,`${viewport.width}: horizontal overflow ${state.overflow}`);assert.ok(state.firstNote.x>=state.roll.x-1&&state.firstNote.right<=state.roll.right+1,`${viewport.width}: Piano Roll note outside roll`);assert.equal(state.assetVersion,'1.0.2');assert.ok(state.resourceNames.some(name=>name.includes('music-studio-dynamic-track-playback.js?v=1.0.2')));
