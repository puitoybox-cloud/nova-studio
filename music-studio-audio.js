...
(function(root){
  ...
  function createSynth(options={}){
    ...
    function playNotes(notes=[],timing={}){
      ...
      const ppq=clamp(timing.ppq||480,24,9600),tempo=clamp(timing.tempo||120,20,400),secondsPerTick=60/(tempo*ppq),lead=.04,playbackStart=ctx.currentTime+lead,fromTick=Math.max(0,Number(timing.startTick)||0);
      const oscillatorStart=diagnostics.oscillatorsCreated;let endTick=fromTick,noteCount=0;for(const note of notes){if(!Number.isFinite(Number(note.startTick))||!Number.isFinite(Number(note.durationTicks)))continue;const noteStart=Math.max(0,Number(note.startTick)),noteEnd=noteStart+Math.max(1,Number(note.durationTicks));if(noteEnd<=fromTick)continue;const startTick=Math.max(noteStart,fromTick),durationTicks=noteEnd-startTick,start=playbackStart+(startTick-fromTick)*secondsPerTick,stop=start+durationTicks*secondsPerTick;voice(note.pitch,note.velocity||100,start,stop,playback,'playback');endTick=Math.max(endTick,noteEnd);noteCount++}
