...
(function(root){
  ...
  function createSynth(options={}){
    ...
    function voice(pitch,velocity,start,stopAt=null,group=null,source='unknown'){
      ...
      diagnostics.voiceCalls++;diagnostics.oscillatorsCreated++;if(source==='live')diagnostics.liveVoicesCreated++;else if(source==='playback')diagnostics.playbackVoicesCreated++;else if(source==='preview')diagnostics.previewVoicesCreated++;
    ...
    function playTracks(tracks=[],timing={}){
      const ctx=ensure();if(!ctx)return{ok:false,noteCount:0,durationMs:0};stopPlayback();
      const ppq=clamp(timing.ppq||480,24,9600),tempo=clamp(timing.tempo||120,20,400),secondsPerTick=60/(tempo*ppq),requestedStart=Number(timing.startTime),lead=Number.isFinite(Number(timing.leadSeconds))?clamp(Number(timing.leadSeconds),0,.25):.04,playbackStart=Number.isFinite(requestedStart)?Math.max(ctx.currentTime,requestedStart):ctx.currentTime+lead,scheduledLead=Math.max(0,playbackStart-ctx.currentTime),fromTick=Math.max(0,Number(timing.startTick)||0),requestedEnd=Number(timing.endTick),toTick=Number.isFinite(requestedEnd)&&requestedEnd>fromTick?requestedEnd:Infinity;
      const oscillatorStart=diagnostics.oscillatorsCreated,scheduledNotes=[];let endTick=fromTick,noteCount=0;for(const track of tracks){const identity={trackId:String(track?.id||''),part:String(track?.part||''),channel:Number(track?.channel)||1,program:track?.program==null?null:Number(track.program)};for(const note of track?.notes||[]){if(!Number.isFinite(Number(note.startTick))||!Number.isFinite(Number(note.durationTicks)))continue;const noteStart=Math.max(0,Number(note.startTick)),noteEnd=noteStart+Math.max(1,Number(note.durationTicks));if(noteEnd<=fromTick||noteStart>=toTick)continue;const startTick=Math.max(noteStart,fromTick),clippedEnd=Math.min(noteEnd,toTick),durationTicks=clippedEnd-startTick,start=playbackStart+(startTick-fromTick)*secondsPerTick,stop=start+durationTicks*secondsPerTick,audible=identity.part==='melody'||identity.part==='bass',scheduled={...identity,noteId:String(note.id||''),pitch:Number(note.pitch),startTick:noteStart,durationTicks:Number(note.durationTicks),velocity:Number(note.velocity)||100,scheduledStartTick:startTick,scheduledDurationTicks:durationTicks,startTime:start,stopTime:stop,audible};scheduledNotes.push(scheduled);if(audible)voice(note.pitch,note.velocity||100,start,stop,playback,'playback');endTick=Math.max(endTick,clippedEnd);noteCount++}}
    ...
    return{supported,unlock,noteOn,noteOff,allNotesOff,playTracks,playNotes,stopPlayback,metronomeClick,stopMetronome,previewNote,dispose,frequency,volume,get diagnostics(){return{...diagnostics}},get context(){return context},get liveVoices(){return live.size},get playingVoices(){return playback.size}};
  }
