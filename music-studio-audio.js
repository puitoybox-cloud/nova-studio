...
(function(root){
  ...
  function createSynth(options={}){
    ...
    function playNotes(notes=[],timing={}){
      ...
      const oscillatorStart=diagnostics.oscillatorsCreated;let endTick=0,noteCount=0;for(const note of notes){if(!Number.isFinite(Number(note.startTick))||!Number.isFinite(Number(note.durationTicks)))continue;const startTick=Math.max(0,Number(note.startTick)),durationTicks=Math.max(1,Number(note.durationTicks)),start=ctx.currentTime+lead+startTick*secondsPerTick,stop=start+durationTicks*secondsPerTick;voice(note.pitch,note.velocity||100,start,stop,playback,'playback');endTick=Math.max(endTick,startTick+durationTicks);noteCount++}
      return{ok:true,noteCount,oscillatorCount:diagnostics.oscillatorsCreated-oscillatorStart,durationMs:(lead+endTick*secondsPerTick+.15)*1000}
    }
