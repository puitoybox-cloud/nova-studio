...
- Story Archiveカードに「キャラクター」「世界観」「場所」「アイテム」「用語」「ストーリー」別の専用入力フォームを追加しました。
- カードごとに確定／未確定フラグを持たせ、確定時は状態も「確定」として扱います。
- カード保存時に変更項目をカード内の変更履歴へ保存し、詳細画面で確認できます。
- 関連カードは複数選択で双方向リンクとして保存され、解除時も相手側から自動で外れます。
- 時系列ラベルと並び順を入力でき、Story Archive画面に時系列表示を追加しました。
- Story Archiveフォルダを作成し、親フォルダを指定した階層管理ができます。
- 既存の `novaStudio_v01` localStorageキーを維持し、既存カード・画像資料・画像セットとの互換性を保ちます。

## Version 0.2.9で追加したこと

- Story Archiveカード詳細の表示順を、カードタイトル、代表画像、画像ギャラリー、基本設定・本文、関連カード、折りたたみ管理情報へ変更しました。
- 画像ギャラリーを大きなグリッド表示にし、画像名、種類、角度、状態、正式採用、Vidu参照を文字で確認できます。
- 画像サムネイル上に角度・状態・正式採用・Vidu参照の文字ラベルを表示し、色だけに依存しない判別にしました。
- 画像拡大表示で左右スワイプ、前後画像切替、拡大、縮小、閉じる、画像情報表示、編集、削除ができます。
- カード一覧では正式採用画像、Vidu参照画像、画像セット基準画像、先頭画像の優先順位で大きな代表画像を表示します。
- カード一覧にタイトル、カテゴリ、状態、画像枚数、関連カード数、更新日時を表示します。
- iPad縦向きではカード詳細上部で代表画像、画像追加、画像セット管理に触れやすくし、横向きでは基本情報と画像ギャラリーの2カラム表示に対応しました。
