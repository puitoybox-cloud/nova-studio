...
Nova Studio は、作品・話数・Scene・制作アプリをひとつのホームから安全に開くための、創作制作環境の入口です。Version 0.2.7 では、Story Archiveの関連カード、複数画像資料、画像セット管理に対応しました。


## Version 0.2.7で追加したこと
...
- 画像セットは、セットID、セット名、対象カードID、説明、含まれる画像資料ID、基準画像、作成日、更新日、並び順を保持します。
- 複数画像選択時は小さなJPEGサムネイルだけを保存し、元画像本体はlocalStorageへ大量保存しません。元画像はiCloudまたは外付けSSDなどの保存場所とファイル名で管理します。
- ギャラリーは、すべて、正式採用、種類別、角度別、状態別、画像セット別、旧案、没案に切り替えできます。
