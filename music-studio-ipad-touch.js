...
(function(root){
  ...
  const INTERACTIVE='.music-midi-note,.music-note-resize,.music-pitch-hit,.music-loop-ruler,.music-loop-selection,.music-loop-handle,.music-time-ruler,button,input,select,textarea,summary';
  ...
  function onTouchStart(event){
    ...
    if(event.touches?.length>=2){
      ...
      pinch={viewport,startDistance:initial,lastDistance:initial};oneFinger=null;
      event.preventDefault();event.stopPropagation();return;
    }
    if(event.touches?.length!==1||event.target?.closest?.(INTERACTIVE)){oneFinger=null;return}
    const p=point(event.touches[0]),scroll=pianoScroll(viewport);
    oneFinger={viewport,scroll,startX:p.x,startY:p.y,lastX:p.x,lastY:p.y,startTop:viewport.scrollTop,startLeft:Number(scroll?.scrollLeft)||0,axis:''};
  }

  function onTouchMove(event){
    if(!coarseLandscape())return;
    if(pinch&&event.touches?.length>=2){
      pinch.lastDistance=distance(event.touches)||pinch.lastDistance;
      event.preventDefault();event.stopPropagation();return;
    }
    ...
    if(!drag.axis&&Math.max(Math.abs(dx),Math.abs(dy))>=5)drag.axis=Math.abs(dy)>=Math.abs(dx)?'vertical':'horizontal';
    if(!drag.axis)return;
  ...
  function onTouchEnd(event){
    if(pinch){event.preventDefault();event.stopPropagation();finishPinch(event);return}
    if(oneFinger){remember(oneFinger.viewport,oneFinger.scroll);oneFinger=null}
  }
  function onTouchCancel(){pinch=null;oneFinger=null}
