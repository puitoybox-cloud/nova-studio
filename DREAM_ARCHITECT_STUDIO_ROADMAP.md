...
| MS-01 | 🔵 ティア確認待ち | Music Studio専用ホーム、15入口、状態表示、安全な仮ページ、ホスト内ルートと単体起動入口を実装 |
| MS-01F | 🔵 ティア確認待ち | Music StudioルートからNova Studio共通ナビを分離し、ホスト内・単体起動の独立表示を修正 |
| MS-02 | 🔵 ティア確認待ち | Version 1プロジェクトの作成・一覧・編集・複製・個別削除・JSON入出力を専用保存領域で実装 |
| MS-03 / MS-03N | 🔵 ティア確認待ち | 専用設定Version 1、自動保存、設定JSON、全体・自動バックアップ、部分復元、ファイル名共通処理、新規プロジェクト初期値連携を実装 |
...
- `docs/music-studio/MS-01_HOME_IMPLEMENTATION.md`
- `docs/music-studio/MS-02_PROJECT_MANAGEMENT.md`
- `docs/music-studio/MS-03_SETTINGS_BACKUP.md`
- `docs/music-studio/MS-03N_NIGHT_IMPLEMENTATION.md`
...
3. ティアがMac・iPad・iPhone相当幅でMS-01ホーム、15入口、状態表示、戻る導線を受入確認する。
4. MS-02のプロジェクト管理、専用保存領域、JSON入出力をMac・iPad・iPhone相当幅で受入確認する。
5. MS-03の専用設定、自動保存、バックアップ、追加統合復元をMac・iPad・iPhone相当幅で受入確認する。
6. 次の正式作業はLogic Pro X連携調査とし、ファイル受渡し、起動方式、保存責務を先に設計する。自動起動・直接操作は別作業とする。
7. URL未設定の既存外部アプリは接続先を正式確認してから登録する。
