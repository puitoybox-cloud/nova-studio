const STORAGE_KEY='novaStudio_v01';let saveTimer=null;let saveStatusTimer=null;
...
function saveState(immediate=false){if(!state.settings?.autoSave&&!immediate)return true;clearTimeout(saveTimer);const run=()=>{try{setSaveStatus('保存中…');state.updatedAt=now();localStorage.setItem(STORAGE_KEY,JSON.stringify(state));setSaveStatus('保存しました');return true}catch(e){console.error(e);setSaveStatus('保存に失敗しました');toast('画像資料の保存容量が不足しています。元画像はiCloudまたはSSDへ保存し、不要なプレビューを整理してください');return false}}; if(immediate)return run();setSaveStatus('保存中…');saveTimer=setTimeout(run,350);return true}
function setSaveStatus(text){const els=document.querySelectorAll('#saveStatus,#modalBody .nova-edit-save-status');els.forEach(el=>{el.textContent=text;if(['保存中…','保存しました','保存に失敗しました'].includes(text))el.classList?.remove('quiet');if(text==='保存済み')el.classList?.add('quiet')});if(saveStatusTimer)clearTimeout(saveStatusTimer);if(text==='保存しました')saveStatusTimer=setTimeout(()=>{document.querySelectorAll('#saveStatus,#modalBody .nova-edit-save-status').forEach(el=>{if(el.textContent==='保存しました'){el.textContent='保存済み';el.classList?.add('quiet')}})},3200)}
