...
function importFile(file){try{NovaSecurity.validateImportFile(file)}catch(e){return toast(e.message)}const reader=new FileReader();reader.onload=()=>{try{const obj=JSON.parse(reader.result);NovaSecurity.validateImportObject(obj);const {data,summary}=analyzeImport(obj);if(!data)throw Error('unsupported');const mode=prompt(summary+'\n\n読み込み方法：add / merge / replace','merge');if(!mode)return;if(mode==='replace'&&!confirm('現在のNova Studioデータを置き換えます。実行前に自動バックアップを書き出します。'))return;if(mode==='replace')exportBackup(true);mergeImportedData(data,['add','merge','replace'].includes(mode)?mode:'merge');state.backupStatus.lastImportedAt=now();saveState(true);render();toast('JSONを読み込みました')}catch(e){toast('不正なJSONです。現在のデータは変更していません。')}};reader.onerror=()=>toast('JSONファイルを読み込めませんでした。現在のデータは変更していません。');reader.readAsText(file)}
